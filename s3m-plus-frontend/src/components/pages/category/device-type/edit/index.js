...
const EditDeviceType = () => {
    ...
    const handleDetail = async () => {
        let res = await DeviceTypeService.getDeviceTypeById(param.id);
        if (res.status === 200) {
            setDataObjectUpdate(res.data);
            let resObjectTypeIds = await ObjectTypeService.getObjectTypeByIds(res.data.objectTypeIds);
            setObjectTypeSelected(resObjectTypeIds.data);
        };
    ...
    const formik = useFormik({
        ...
        onSubmit: async (data) => {
            ...
            let res = await DeviceTypeService.updateDeviceType(data);
            if (res.status === 400) {
            ...
            if (res.status === 200) {
                $.alert({
                    ...
                    content: 'Chỉnh sửa loại thiết bị thành công!'
                });
                history.push({
                ...
                setError('Lỗi phát sinh khi chỉnh sửa loại thiết bị mới')
            }
    ...
    const setObjectTypeIds = async () => {
        let ids = [];
        objectTypeSelected.forEach(objectType => {
            ids.push(objectType.id);
        })

        setDataObjectUpdate({ ...dataObjectUpdate, objectTypeIds: ids.toString() });
    }
    ...
    useEffect(() => {
        document.title = t('content.category.device_type.edit.title');
        handleDetail();
        getObjectType();
    }, []);
        ...
        <div id="page-body">
            {
                ...
                <div className="alert alert-danger" role="alert">
                    <p className="m-0 p-0">{t('content.category.device_type.edit.error_edit')}</p>
                </div>
            }
            {
            ...
            <form onSubmit={formik.handleSubmit}>
                <div id="main-content">
                    <div id="main-title">
                        <h5 className="d-block mb-0 float-left text-uppercase"><i className="far fa-clone"></i> &nbsp;{t('content.category.device_type.edit.title')}</h5>
                    </div>
                    <table className="table table-input">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <input type="text" name="name" className="form-control" defaultValue={dataObjectUpdate.name} onChange={onInputChange} />
                                </td>
                            </tr>
                            ...
                            {
                                dataObjectUpdate.img != "" && dataObjectUpdate.img != null ?
                                    <tr>
                                        <th width="150px">
                    ...
                    <div id="main-button">
                        <button type="submit" id="submitButton" className="btn btn-outline-secondary btn-agree mr-1" onClick={() => setObjectTypeIds()}>
