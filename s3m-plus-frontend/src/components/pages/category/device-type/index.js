...
import DeviceTypeRouters from "./device-type.routes";
...
const DeviceTypeComponent = () => {
    ...
    const [devicePath] = useState(useAppStore(state => state.categoryPath).find(f => f.path.includes("/category/device-type")));
    ...
    return (
        <Switch>
            {
                role === "ROLE_ADMIN" ? (
                    DeviceTypeRouters.map((route, i) => {
                        let permissionActions = null;
                        if(route?.path === "/category/device-type"){
                            permissionActions = {
                        ...
                        return ((route?.path === "/category/device-type") ) ?  (
                    ...
                    DeviceTypeRouters.map((route) => {
                        ...
                        return devicePath.actions.map((act, i) => {
                            if(route.action === act){
                                return ((path?.path === "/category/device-type") ) ? (
