...
let KHOANG1 = {
    ID: "idkhg-",
    ID_CIRCLE_CONNECT: "khgc",
    ID_CONNECT_TOP_MARKER: "khgct-",
    ID_CONNECT_LEFT_MARKER: "khgcl-",
    ID_CONNECT_RIGHT_MARKER: "khgcr-",
    ID_CONNECT_BOTTOM_MARKER: "khgcb-",

    ID_STATUS_KHOANG1: "iskhg-",
    ID_PATH_1_KHOANG1: "ip1khg-",
    ID_PATH_2_KHOANG1: "ip2khg-",
    ID_REMOVE: "irmkhg-",
    ID_DETAIL: "idtkhg-",
    ID_FOCUS: "ifkhg-",
    ...
    ID_KHOANG1_VALUE_T: "ivtkhg-",
    ID_KHOANG1_VALUE_H: "ivhkhg-",
    ID_KHOANG1_VALUE_P: "ivpkhg-",
    ...
    collect: function () {
        var _tagName = document.getElementById("khoang1-tag-name").value;
        var _deviceName = document.getElementById("khoang1-device-name").value;
        var _description = document.getElementById("khoang1-description").value;
        var _deviceCaculator = Number($("input[name=khoang1_caculator]").filter(":checked").val());
    ...
    drawLoad: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            .attr("id", function (d) { return (KHOANG1.ID + d.id); })
        ...
        var _path1 = _gCard.datum(card).append("path")
            .attr("id", function (d) { return (KHOANG1.ID_PATH_1_KHOANG1 + d.deviceId); })
            .attr("d", CONSTANTS.KHOANGDOLUONG.path1)
        ...
        var _deviceName = _svgDeviceName.datum(card).append("text")
            ...
            .attr("text-anchor", "middle")
            .attr("font-size", "15")
            .text(function (d) { return d.deviceName > 27 ? d.deviceName.substring(0, 25) + "..." : d.deviceName });
        ...
        var _rect = _gCard.datum(card).append("rect")
            .attr("id", function (d) { return (KHOANG1.ID_FOCUS + d.id); })
            .attr("x", "-30")
            .attr("y", "-30")
            .attr("width", "238")
            .attr("height", "238")
            .attr("fill", "transparent")
            .on("click", KHOANG1.detail);
    ...
    draw: function (card) {
        ...
        var _cardLeftMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return KHOANG1.ID_CONNECT_LEFT_MARKER + d.id; })
            ...
            .attr("opacity", 0.5)
            .attr("cursor", "pointer")
            .attr("marker", KHOANG1.ID_CONNECT_LEFT_MARKER)
            .attr("fill", "#237ba0")
            .call(KHOANG1.connect);

        var _cardRightMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return KHOANG1.ID_CONNECT_RIGHT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 178)
            ...
            .attr("opacity", 0.5)
            .attr("cursor", "pointer")
            .attr("marker", KHOANG1.ID_CONNECT_RIGHT_MARKER)
            .attr("fill", "#237ba0")
            .call(KHOANG1.connect);

        var _gX = _gCard.datum(card).append("g")
            .attr("id", function (d) { return (KHOANG1.ID_REMOVE + d.id); })
            .attr("transform", "translate(-20,-40)")
            .attr("opacity", "1")
            .attr("cursor", "pointer")
            .on("click", KHOANG1.delete);
    ...
    drag: d3.drag()
        .on("start", function (i, d) {
            ...
            var _oCard = KHOANG1.get(d.id);
            document.getElementById("khoang1-device-name").value = _oCard.deviceName;
            document.getElementById("khoang1-tag-name").value = _oCard.tagName;
            document.getElementById("khoang1-description").value = _oCard.description;

            if (Number(_oCard.caculator) == 1) {
                document.getElementById("khoang1-caculator-1").checked = true;
            } else {
                document.getElementById("khoang1-caculator-0").checked = true;
            };
        ...
        .on("drag", function (i, d) {
            var _gCard = d3.select("g[id=" + KHOANG1.ID + d.id + "]");
            var _circleLeftMaker = d3.select("circle[id=" + KHOANG1.ID_CONNECT_LEFT_MARKER + d.id + "]");
            var _circleRightMaker = d3.select("circle[id=" + KHOANG1.ID_CONNECT_RIGHT_MARKER + d.id + "]");

            d3.select(this).attr("transform", "translate(" + (d.x) + "," + (d.y) + ")");
            d.x = i.x;
            d.y = i.y;
            KHOANG1.moveArrows(d.id);
            ...
            if (g_connect.cardId == "") {
                d3.selectAll("circle[id^=" + METER.ID_CIRCLE_CONNECT + "]").each(function (c) {
                    ...
                    if (_isOverlapL || _isOverlapR) {
                        ...
                        if (_isOverlapL) {
                            g_connect.circleFrom = KHOANG1.ID_CONNECT_LEFT_MARKER;
                        } else if (_isOverlapR) {
                            g_connect.circleFrom = KHOANG1.ID_CONNECT_RIGHT_MARKER;
                        };
                    };
                });
    ...
    connect: d3.drag()
        .on("start", function (i, d) {
            ...
            var _gArrow = g_svg.datum(KHOANG1.directArrow).append('g')
            ...
            var _arrowLine = _gArrow.datum(KHOANG1.directArrow).append("line")
            ...
            var _arrowStartPoint = _gArrow.datum(KHOANG1.directArrow).append('circle')
            ...
            var _arrowEndPoint = _gArrow.datum(KHOANG1.directArrow).append('circle')
        ...
        .on("drag", function (event, d) {
            ...
            var _gArrow = d3.select("g[id=" + ARROW.ID + KHOANG1.directArrow.id + "]");
            var _arrowLine = d3.select("line[id=" + ARROW.ID_LINE + KHOANG1.directArrow.id + "]");
            var _arrowEndPoint = d3.select("circle[id=" + ARROW.ID_LINE_POINT_END + KHOANG1.directArrow.id + "]");
        ...
        .on("end", function (i, d) {
            ...
            if (g_connect.attachFlag && g_connect.id != "") {
                ...
                KHOANG1.directArrow.to = g_connect.id;
                KHOANG1.directArrow.toMarker = g_connect.attachMarker;
                KHOANG1.directArrow.x2 = _x;
                KHOANG1.directArrow.y2 = _y;
                DATA.aArrows.push(KHOANG1.directArrow);
                ARROW.update(KHOANG1.directArrow.id, 1);
            } else {
                d3.select("g[id=" + ARROW.ID + KHOANG1.directArrow.id + "]").remove();
            }
    ...
    remove: function (id) {
        ...
        if (!isMarker) {
            ...
            d3.select("g[id=" + KHOANG1.ID + id + "]").remove();
    ...
    focus: function (id) {
        if (id != null) {
            $('circle[id^=' + KHOANG1.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + KHOANG1.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 1);
            $('g[id^=' + KHOANG1.ID_REMOVE + id + ']').attr("opacity", 1);
        }
    },

    outFocus: function (id) {
        if (id != null) {
            $('circle[id^=' + KHOANG1.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + KHOANG1.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 0);
            $('g[id^=' + KHOANG1.ID_REMOVE + id + ']').attr("opacity", 0)
        } else {
            $('circle[id^=' + KHOANG1.ID_CONNECT_LEFT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + KHOANG1.ID_CONNECT_RIGHT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('g[id^=' + KHOANG1.ID_REMOVE + ']').each(function () {
                $(this).attr("opacity", 0);
            });
        }
    },
    ...
    moveArrows: function (id) {
        ...
        $.each(_attachArrows, function (i, data) {
            ...
            var _gCard = d3.select("g[id=" + KHOANG1.ID + id + "]");
    ...
    update: function (id) {
        var _oCurrent = KHOANG1.get(id);

        if (typeof _oCurrent !== "undefined" && _oCurrent.type == "khoang1") {
            var _deviceCaculator = Number($("input[name=khoang1_caculator]").filter(":checked").val());
            var _tagName = document.getElementById("khoang1-tag-name").value;
            var _description = document.getElementById("khoang1-description").value;
            var _oCard = {
