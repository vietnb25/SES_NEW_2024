...
let KHOANGDODEMSYMBOL = {
    ID: "idkddsb-",
    ID_CIRCLE_CONNECT: "kddsbc",
    ID_CONNECT_TOP_MARKER: "kddsbct-",
    ID_CONNECT_LEFT_MARKER: "kddsbcl-",
    ID_CONNECT_RIGHT_MARKER: "kddsbcr-",
    ID_CONNECT_BOTTOM_MARKER: "kddsbcb-",

    ID_PATH_1_KHOANGDODEMSYMBOL: "ip1kddsb-",
    ID_PATH_2_KHOANGDODEMSYMBOL: "ip2kddsb-",
    ID_REMOVE: "irmkddsb-",
    ...
    collect: function () {
        var _deviceName = document.getElementById("khoang-do-dem-symbol-device-name").value;
        var _description = document.getElementById("khoang-do-dem-symbol-description").value;

        var _khoangdodemsymbol = {
            "id": Date.now(),
            "deviceName": _deviceName,
            "description": _description,
            "type": "khoangdodemsymbol",
        };
        return _khoangdodemsymbol;
    },

    create: function () {
        var _oCard = KHOANGDODEMSYMBOL.collect();
        DATA.aCards.push(_oCard);
        KHOANGDODEMSYMBOL.draw(_oCard);
    },
    ...
    drawLoad: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            .attr("id", function (d) { return (KHOANGDODEMSYMBOL.ID + d.id); })
        ...
        var _path1 = _gCard.datum(card).append("path")
            .attr("id", function (d) { return (KHOANGDODEMSYMBOL.ID_PATH_1_KHOANGDODEMSYMBOL + d.deviceId); })
            .attr("d", CONSTANTS.KHOANGDODEMSYMBOL.path1)
        ...
        var _path2 = _gCard.datum(card).append("path")
            .attr("id", function (d) { return (KHOANGDODEMSYMBOL.ID_PATH_2_KHOANGDODEMSYMBOL + d.deviceId); })
            .attr("d", CONSTANTS.KHOANGDODEMSYMBOL.path2)
            .attr("style", "stroke:none;fill-rule:evenodd;fill:rgb(4.705882%,13.72549%,14.117647%);fill-opacity:1;");
    ...
    draw: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            ...
            .on("click", KHOANGDODEMSYMBOL.select)
            .call(KHOANGDODEMSYMBOL.drag);
        ...
        var _cardTopMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return KHOANGDODEMSYMBOL.ID_CONNECT_TOP_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", KHOANGDODEMSYMBOL.DEFAULT_WIDTH / 2)
            .attr("cy", 2)
            .attr("opacity", 0.5)
            .attr("cursor", "pointer")
            .attr("marker", KHOANGDODEMSYMBOL.ID_CONNECT_TOP_MARKER)
            .attr("fill", "#237ba0")
            .call(KHOANGDODEMSYMBOL.connect);

        var _cardLeftMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return KHOANGDODEMSYMBOL.ID_CONNECT_LEFT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 2)
            .attr("cy", KHOANGDODEMSYMBOL.DEFAULT_HEIGHT / 2)
            .attr("opacity", 0.5)
            .attr("cursor", "pointer")
            .attr("marker", KHOANGDODEMSYMBOL.ID_CONNECT_LEFT_MARKER)
            .attr("fill", "#237ba0")
            .call(KHOANGDODEMSYMBOL.connect);

        var _cardRightMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return KHOANGDODEMSYMBOL.ID_CONNECT_RIGHT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", KHOANGDODEMSYMBOL.DEFAULT_WIDTH - 2)
            ...
            .attr("opacity", 0.5)
            .attr("cursor", "pointer")
            .attr("marker", KHOANGDODEMSYMBOL.ID_CONNECT_RIGHT_MARKER)
            .attr("fill", "#237ba0")
            .call(KHOANGDODEMSYMBOL.connect);

        var _cardBottomMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return KHOANGDODEMSYMBOL.ID_CONNECT_BOTTOM_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", KHOANGDODEMSYMBOL.DEFAULT_WIDTH / 2)
            ...
            .attr("opacity", 0.5)
            .attr("cursor", "pointer")
            .attr("marker", KHOANGDODEMSYMBOL.ID_CONNECT_BOTTOM_MARKER)
            .attr("fill", "#237ba0")
            .call(KHOANGDODEMSYMBOL.connect);

        var _gX = _gCard.datum(card).append("g")
            .attr("id", function (d) { return (KHOANGDODEMSYMBOL.ID_REMOVE + d.id); })
            .attr("transform", "translate(-20,-40)")
            .attr("opacity", "1")
            .attr("cursor", "pointer")
            .on("click", KHOANGDODEMSYMBOL.delete);
    ...
    drag: d3.drag()
        .on("start", function (i, d) {
            ...
            var _oCard = KHOANGDODEMSYMBOL.get(d.id);
            document.getElementById("khoang-do-dem-symbol-device-name").value = _oCard.deviceName;
            document.getElementById("khoang-do-dem-symbol-description").value = _oCard.description;
        ...
        .on("drag", function (i, d) {
            var _gCard = d3.select("g[id=" + KHOANGDODEMSYMBOL.ID + d.id + "]");
            var _circleTopMaker = d3.select("circle[id=" + KHOANGDODEMSYMBOL.ID_CONNECT_TOP_MARKER + d.id + "]");
            var _circleLeftMaker = d3.select("circle[id=" + KHOANGDODEMSYMBOL.ID_CONNECT_LEFT_MARKER + d.id + "]");
            var _circleRightMaker = d3.select("circle[id=" + KHOANGDODEMSYMBOL.ID_CONNECT_RIGHT_MARKER + d.id + "]");
            var _circleBottomMaker = d3.select("circle[id=" + KHOANGDODEMSYMBOL.ID_CONNECT_BOTTOM_MARKER + d.id + "]");

            d3.select(this).attr("transform", "translate(" + (d.x) + "," + (d.y) + ")");
            d.x = i.x;
            d.y = i.y;
            KHOANGDODEMSYMBOL.moveArrows(d.id);
            ...
            if (g_connect.cardId == "") {
                d3.selectAll("circle[id^=" + METER.ID_CIRCLE_CONNECT + "]").each(function (c) {
                    ...
                    if (_isOverlapT || _isOverlapL || _isOverlapR || _isOverlapB) {
                        ...
                        if (_isOverlapT) {
                            g_connect.circleFrom = KHOANGDODEMSYMBOL.ID_CONNECT_TOP_MARKER;
                        } else if (_isOverlapL) {
                            g_connect.circleFrom = KHOANGDODEMSYMBOL.ID_CONNECT_LEFT_MARKER;
                        } else if (_isOverlapR) {
                            g_connect.circleFrom = KHOANGDODEMSYMBOL.ID_CONNECT_RIGHT_MARKER;
                        } else if (_isOverlapB) {
                            g_connect.circleFrom = KHOANGDODEMSYMBOL.ID_CONNECT_BOTTOM_MARKER;
                        };
                    };
                });
    ...
    connect: d3.drag()
        .on("start", function (i, d) {
            ...
            var _gArrow = g_svg.datum(KHOANGDODEMSYMBOL.directArrow).append('g')
            ...
            var _arrowLine = _gArrow.datum(KHOANGDODEMSYMBOL.directArrow).append("line")
            ...
            var _arrowStartPoint = _gArrow.datum(KHOANGDODEMSYMBOL.directArrow).append('circle')
            ...
            var _arrowEndPoint = _gArrow.datum(KHOANGDODEMSYMBOL.directArrow).append('circle')
        ...
        .on("drag", function (event, d) {
            ...
            var _gArrow = d3.select("g[id=" + ARROW.ID + KHOANGDODEMSYMBOL.directArrow.id + "]");
            var _arrowLine = d3.select("line[id=" + ARROW.ID_LINE + KHOANGDODEMSYMBOL.directArrow.id + "]");
            var _arrowEndPoint = d3.select("circle[id=" + ARROW.ID_LINE_POINT_END + KHOANGDODEMSYMBOL.directArrow.id + "]");
        ...
        .on("end", function (i, d) {
            ...
            if (g_connect.attachFlag && g_connect.id != "") {
                ...
                if (g_connect.type == "meter") {
                    ...
                    KHOANGDODEMSYMBOL.directArrow.toType = METER.ID;
                } else if (g_connect.type == "mba") {
                    _gCard = d3.select("g[id=" + MBA.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = MBA.ID;
                } else if (g_connect.type == "label") {
                    _gCard = d3.select("g[id=" + LABEL.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = LABEL.ID;
                } else if (g_connect.type == "rmu") {
                    _gCard = d3.select("g[id=" + RMU.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = RMU.ID;
                } else if (g_connect.type == "mc") {
                    _gCard = d3.select("g[id=" + MC.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = MC.ID;
                } else if (g_connect.type == "mchor") {
                    _gCard = d3.select("g[id=" + MCHOR.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = MCHOR.ID;
                } else if (g_connect.type == "feeder") {
                    _gCard = d3.select("g[id=" + FEEDER.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = FEEDER.ID;
                } else if (g_connect.type == "ultility") {
                    _gCard = d3.select("g[id=" + ULTILITY.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = ULTILITY.ID;
                } else if (g_connect.type == "busbar") {
                    _gCard = d3.select("g[id=" + BUSBAR.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = BUSBAR.ID;
                } else if (g_connect.type == "busbarver") {
                    _gCard = d3.select("g[id=" + BUSBARVER.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = BUSBARVER.ID;
                } else if (g_connect.type == "stmv") {
                    _gCard = d3.select("g[id=" + STMV.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = STMV.ID;
                } else if (g_connect.type == "sgmv") {
                    _gCard = d3.select("g[id=" + SGMV.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = SGMV.ID;
                } else if (g_connect.type == "inverter") {
                    _gCard = d3.select("g[id=" + INVERTER.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = INVERTER.ID;
                } else if (g_connect.type == "invertersymbol") {
                    _gCard = d3.select("g[id=" + INVERTERSYMBOL.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = INVERTERSYMBOL.ID;
                } else if (g_connect.type == "combiner") {
                    _gCard = d3.select("g[id=" + COMBINER.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = COMBINER.ID;
                } else if (g_connect.type == "combinersymbol") {
                    _gCard = d3.select("g[id=" + COMBINERSYMBOL.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = COMBINERSYMBOL.ID;
                } else if (g_connect.type == "panel") {
                    _gCard = d3.select("g[id=" + PANEL.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = PANEL.ID;
                } else if (g_connect.type == "panelsymbol") {
                    _gCard = d3.select("g[id=" + PANELSYMBOL.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = PANELSYMBOL.ID;
                } else if (g_connect.type == "string") {
                    _gCard = d3.select("g[id=" + STRING.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = STRING.ID;
                } else if (g_connect.type == "stringsymbol") {
                    _gCard = d3.select("g[id=" + STRINGSYMBOL.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = STRINGSYMBOL.ID;
                } else if (g_connect.type == "weather") {
                    _gCard = d3.select("g[id=" + WEATHER.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = WEATHER.ID;
                } else if (g_connect.type == "weathersymbol") {
                    _gCard = d3.select("g[id=" + WEATHERSYMBOL.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = WEATHERSYMBOL.ID;
                } else if (g_connect.type == "ups") {
                    _gCard = d3.select("g[id=" + UPS.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = UPS.ID;
                } else if (g_connect.type == "khoang1") {
                    _gCard = d3.select("g[id=" + KHOANG1.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = KHOANG1.ID;
                } else if (g_connect.type == "khoang1symbol") {
                    _gCard = d3.select("g[id=" + KHOANG1SYMBOL.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = KHOANG1SYMBOL.ID;
                } else if (g_connect.type == "khoangcap") {
                    _gCard = d3.select("g[id=" + KHOANGCAP.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = KHOANGCAP.ID;
                } else if (g_connect.type == "khoangcapsymbol") {
                    _gCard = d3.select("g[id=" + KHOANGCAPSYMBOL.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = KHOANGCAPSYMBOL.ID;
                } else if (g_connect.type == "khoangchi") {
                    _gCard = d3.select("g[id=" + KHOANGCHI.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = KHOANGCHI.ID;
                } else if (g_connect.type == "khoangdodem") {
                    _gCard = d3.select("g[id=" + KHOANGDODEM.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = KHOANGDODEM.ID;
                } else if (g_connect.type == "khoangmaycat") {
                    _gCard = d3.select("g[id=" + KHOANGMAYCAT.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = KHOANGMAYCAT.ID;
                } else if (g_connect.type == "khoangmaycatsymbol") {
                    _gCard = d3.select("g[id=" + KHOANGMAYCATSYMBOL.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = KHOANGMAYCATSYMBOL.ID;
                } else if (g_connect.type == "khoangthanhcai") {
                    _gCard = d3.select("g[id=" + KHOANGTHANHCAI.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = KHOANGTHANHCAI.ID;
                } else if (g_connect.type == "khoangthanhcaisymbol") {
                    _gCard = d3.select("g[id=" + KHOANGTHANHCAISYMBOL.ID + g_connect.id + "]");
                    KHOANGDODEMSYMBOL.directArrow.toType = KHOANGTHANHCAISYMBOL.ID;
                };
                ...
                KHOANGDODEMSYMBOL.directArrow.to = g_connect.id;
                KHOANGDODEMSYMBOL.directArrow.toMarker = g_connect.attachMarker;
                KHOANGDODEMSYMBOL.directArrow.x2 = _x;
                KHOANGDODEMSYMBOL.directArrow.y2 = _y;
                DATA.aArrows.push(KHOANGDODEMSYMBOL.directArrow);
                ARROW.update(KHOANGDODEMSYMBOL.directArrow.id, 1);
            } else {
                d3.select("g[id=" + ARROW.ID + KHOANGDODEMSYMBOL.directArrow.id + "]").remove();
            }
    ...
    remove: function (id) {
        ...
        if (!isMarker) {
            d3.select("g[id=" + KHOANGDODEMSYMBOL.ID + id + "]").remove();
    ...
    focus: function (id) {
        if (id != null) {
            $('circle[id^=' + KHOANGDODEMSYMBOL.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + KHOANGDODEMSYMBOL.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + KHOANGDODEMSYMBOL.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + KHOANGDODEMSYMBOL.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 1);
            $('g[id^=' + KHOANGDODEMSYMBOL.ID_REMOVE + id + ']').attr("opacity", 1);
        }
    },

    outFocus: function (id) {
        if (id != null) {
            $('circle[id^=' + KHOANGDODEMSYMBOL.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + KHOANGDODEMSYMBOL.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + KHOANGDODEMSYMBOL.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + KHOANGDODEMSYMBOL.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 0);
            $('g[id^=' + KHOANGDODEMSYMBOL.ID_REMOVE + id + ']').attr("opacity", 0)
        } else {
            $('circle[id^=' + KHOANGDODEMSYMBOL.ID_CONNECT_TOP_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + KHOANGDODEMSYMBOL.ID_CONNECT_LEFT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + KHOANGDODEMSYMBOL.ID_CONNECT_RIGHT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + KHOANGDODEMSYMBOL.ID_CONNECT_BOTTOM_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('g[id^=' + KHOANGDODEMSYMBOL.ID_REMOVE + ']').each(function () {
                $(this).attr("opacity", 0);
            });
        }
    },
    ...
    moveArrows: function (id) {
        ...
        $.each(_attachArrows, function (i, data) {
            ...
            var _gCard = d3.select("g[id=" + KHOANGDODEMSYMBOL.ID + id + "]");
    ...
    update: function (id) {
        var _oCurrent = KHOANGDODEMSYMBOL.get(id);

        if (typeof _oCurrent !== "undefined" && _oCurrent.type == "khoangdodemsymbol") {
            var _deviceName = document.getElementById("khoang-do-dem-symbol-device-name").value;
            var _description = document.getElementById("khoang-do-dem-symbol-description").value;
            var _oCard = {
                "id": _oCurrent.id,
...
export default KHOANGDODEMSYMBOL;