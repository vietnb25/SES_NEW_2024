...
let KHOANGCAPSYMBOL = {
    ID: "idkhgcsb-",
    ID_CIRCLE_CONNECT: "kgcsbc",
    ID_CONNECT_TOP_MARKER: "kgcsbct-",
    ID_CONNECT_LEFT_MARKER: "kgcsbcl-",
    ID_CONNECT_RIGHT_MARKER: "kgcsbcr-",
    ID_CONNECT_BOTTOM_MARKER: "kgcsbcb-",

    ID_PATH_1_KHOANGCAPSYMBOL: "ip1khgcsb-",
    ID_PATH_2_KHOANGCAPSYMBOL: "ip2khgcsb-",
    ID_REMOVE: "irmkhgcsb-",
    ...
    collect: function () {
        var _deviceName = document.getElementById("khoang-cap-symbol-device-name").value;
        var _description = document.getElementById("khoang-cap-symbol-description").value;

        var _khoangcapsymbol = {
            "id": Date.now(),
            "deviceName": _deviceName,
            "description": _description,
            "type": "khoangcapsymbol",
        };
        return _khoangcapsymbol;
    },

    create: function () {
        var _oCard = KHOANGCAPSYMBOL.collect();
        DATA.aCards.push(_oCard);
        KHOANGCAPSYMBOL.draw(_oCard);
    },
    ...
    drawLoad: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            .attr("id", function (d) { return (KHOANGCAPSYMBOL.ID + d.id); })
        ...
        var _path1 = _gCard.datum(card).append("path")
            .attr("id", function (d) { return (KHOANGCAPSYMBOL.ID_PATH_1_KHOANGCAPSYMBOL + d.deviceId); })
            .attr("d", CONSTANTS.KHOANGCAPSYMBOL.path1)
    ...
    draw: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            ...
            .on("click", KHOANGCAPSYMBOL.select)
            .call(KHOANGCAPSYMBOL.drag);
        ...
        var _cardTopMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return KHOANGCAPSYMBOL.ID_CONNECT_TOP_MARKER + d.id; })
            ...
            .attr("opacity", 0.5)
            .attr("cursor", "pointer")
            .attr("marker", KHOANGCAPSYMBOL.ID_CONNECT_TOP_MARKER)
            .attr("fill", "#237ba0")
            .call(KHOANGCAPSYMBOL.connect);

        var _cardBottomMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return KHOANGCAPSYMBOL.ID_CONNECT_BOTTOM_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 52.9)
            ...
            .attr("opacity", 0.5)
            .attr("cursor", "pointer")
            .attr("marker", KHOANGCAPSYMBOL.ID_CONNECT_BOTTOM_MARKER)
            .attr("fill", "#237ba0")
            .call(KHOANGCAPSYMBOL.connect);

        var _gX = _gCard.datum(card).append("g")
            .attr("id", function (d) { return (KHOANGCAPSYMBOL.ID_REMOVE + d.id); })
            .attr("transform", "translate(-20,-40)")
            .attr("opacity", "1")
            .attr("cursor", "pointer")
            .on("click", KHOANGCAPSYMBOL.delete);
    ...
    drag: d3.drag()
        .on("start", function (i, d) {
            ...
            var _oCard = KHOANGCAPSYMBOL.get(d.id);
            document.getElementById("khoang-cap-symbol-device-name").value = _oCard.deviceName;
            document.getElementById("khoang-cap-symbol-description").value = _oCard.description;
        ...
        .on("drag", function (i, d) {
            var _gCard = d3.select("g[id=" + KHOANGCAPSYMBOL.ID + d.id + "]");
            var _circleTopMaker = d3.select("circle[id=" + KHOANGCAPSYMBOL.ID_CONNECT_TOP_MARKER + d.id + "]");
            var _circleBottomMaker = d3.select("circle[id=" + KHOANGCAPSYMBOL.ID_CONNECT_BOTTOM_MARKER + d.id + "]");

            d3.select(this).attr("transform", "translate(" + (d.x) + "," + (d.y) + ")");
            d.x = i.x;
            d.y = i.y;
            KHOANGCAPSYMBOL.moveArrows(d.id);
            ...
            if (g_connect.cardId == "") {
                d3.selectAll("circle[id^=" + METER.ID_CIRCLE_CONNECT + "]").each(function (c) {
                    ...
                    if (_isOverlapT || _isOverlapB) {
                        ...
                        if (_isOverlapT) {
                            g_connect.circleFrom = KHOANGCAPSYMBOL.ID_CONNECT_TOP_MARKER;
                        } else if (_isOverlapB) {
                            g_connect.circleFrom = KHOANGCAPSYMBOL.ID_CONNECT_BOTTOM_MARKER;
                        };
                    };
                });
    ...
    connect: d3.drag()
        .on("start", function (i, d) {
            ...
            var _gArrow = g_svg.datum(KHOANGCAPSYMBOL.directArrow).append('g')
            ...
            var _arrowLine = _gArrow.datum(KHOANGCAPSYMBOL.directArrow).append("line")
            ...
            var _arrowStartPoint = _gArrow.datum(KHOANGCAPSYMBOL.directArrow).append('circle')
            ...
            var _arrowEndPoint = _gArrow.datum(KHOANGCAPSYMBOL.directArrow).append('circle')
        ...
        .on("drag", function (event, d) {
            ...
            var _gArrow = d3.select("g[id=" + ARROW.ID + KHOANGCAPSYMBOL.directArrow.id + "]");
            var _arrowLine = d3.select("line[id=" + ARROW.ID_LINE + KHOANGCAPSYMBOL.directArrow.id + "]");
            var _arrowEndPoint = d3.select("circle[id=" + ARROW.ID_LINE_POINT_END + KHOANGCAPSYMBOL.directArrow.id + "]");
        ...
        .on("end", function (i, d) {
            ...
            if (g_connect.attachFlag && g_connect.id != "") {
                ...
                if (g_connect.type == "meter") {
                    ...
                    KHOANGCAPSYMBOL.directArrow.toType = METER.ID;
                } else if (g_connect.type == "mba") {
                    _gCard = d3.select("g[id=" + MBA.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = MBA.ID;
                } else if (g_connect.type == "label") {
                    _gCard = d3.select("g[id=" + LABEL.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = LABEL.ID;
                } else if (g_connect.type == "rmu") {
                    _gCard = d3.select("g[id=" + RMU.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = RMU.ID;
                } else if (g_connect.type == "mc") {
                    _gCard = d3.select("g[id=" + MC.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = MC.ID;
                } else if (g_connect.type == "mchor") {
                    _gCard = d3.select("g[id=" + MCHOR.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = MCHOR.ID;
                } else if (g_connect.type == "feeder") {
                    _gCard = d3.select("g[id=" + FEEDER.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = FEEDER.ID;
                } else if (g_connect.type == "ultility") {
                    _gCard = d3.select("g[id=" + ULTILITY.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = ULTILITY.ID;
                } else if (g_connect.type == "busbar") {
                    _gCard = d3.select("g[id=" + BUSBAR.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = BUSBAR.ID;
                } else if (g_connect.type == "busbarver") {
                    _gCard = d3.select("g[id=" + BUSBARVER.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = BUSBARVER.ID;
                } else if (g_connect.type == "stmv") {
                    _gCard = d3.select("g[id=" + STMV.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = STMV.ID;
                } else if (g_connect.type == "sgmv") {
                    _gCard = d3.select("g[id=" + SGMV.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = SGMV.ID;
                } else if (g_connect.type == "inverter") {
                    _gCard = d3.select("g[id=" + INVERTER.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = INVERTER.ID;
                } else if (g_connect.type == "invertersymbol") {
                    _gCard = d3.select("g[id=" + INVERTERSYMBOL.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = INVERTERSYMBOL.ID;
                } else if (g_connect.type == "combiner") {
                    _gCard = d3.select("g[id=" + COMBINER.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = COMBINER.ID;
                } else if (g_connect.type == "combinersymbol") {
                    _gCard = d3.select("g[id=" + COMBINERSYMBOL.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = COMBINERSYMBOL.ID;
                } else if (g_connect.type == "panel") {
                    _gCard = d3.select("g[id=" + PANEL.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = PANEL.ID;
                } else if (g_connect.type == "panelsymbol") {
                    _gCard = d3.select("g[id=" + PANELSYMBOL.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = PANELSYMBOL.ID;
                } else if (g_connect.type == "string") {
                    _gCard = d3.select("g[id=" + STRING.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = STRING.ID;
                } else if (g_connect.type == "stringsymbol") {
                    _gCard = d3.select("g[id=" + STRINGSYMBOL.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = STRINGSYMBOL.ID;
                } else if (g_connect.type == "weather") {
                    _gCard = d3.select("g[id=" + WEATHER.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = WEATHER.ID;
                } else if (g_connect.type == "weathersymbol") {
                    _gCard = d3.select("g[id=" + WEATHERSYMBOL.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = WEATHERSYMBOL.ID;
                } else if (g_connect.type == "ups") {
                    _gCard = d3.select("g[id=" + UPS.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = UPS.ID;
                } else if (g_connect.type == "khoang1") {
                    _gCard = d3.select("g[id=" + KHOANG1.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = KHOANG1.ID;
                } else if (g_connect.type == "khoang1symbol") {
                    _gCard = d3.select("g[id=" + KHOANG1SYMBOL.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = KHOANG1SYMBOL.ID;
                } else if (g_connect.type == "khoangcap") {
                    _gCard = d3.select("g[id=" + KHOANGCAP.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = KHOANGCAP.ID;
                } else if (g_connect.type == "khoangchi") {
                    _gCard = d3.select("g[id=" + KHOANGCHI.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = KHOANGCHI.ID;
                } else if (g_connect.type == "khoangdodem") {
                    _gCard = d3.select("g[id=" + KHOANGDODEM.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = KHOANGDODEM.ID;
                } else if (g_connect.type == "khoangdodemsymbol") {
                    _gCard = d3.select("g[id=" + KHOANGDODEMSYMBOL.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = KHOANGDODEMSYMBOL.ID;
                } else if (g_connect.type == "khoangmaycat") {
                    _gCard = d3.select("g[id=" + KHOANGMAYCAT.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = KHOANGMAYCAT.ID;
                } else if (g_connect.type == "khoangmaycatsymbol") {
                    _gCard = d3.select("g[id=" + KHOANGMAYCATSYMBOL.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = KHOANGMAYCATSYMBOL.ID;
                } else if (g_connect.type == "khoangthanhcai") {
                    _gCard = d3.select("g[id=" + KHOANGTHANHCAI.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = KHOANGTHANHCAI.ID;
                } else if (g_connect.type == "khoangthanhcaisymbol") {
                    _gCard = d3.select("g[id=" + KHOANGTHANHCAISYMBOL.ID + g_connect.id + "]");
                    KHOANGCAPSYMBOL.directArrow.toType = KHOANGTHANHCAISYMBOL.ID;
                };
                ...
                KHOANGCAPSYMBOL.directArrow.to = g_connect.id;
                KHOANGCAPSYMBOL.directArrow.toMarker = g_connect.attachMarker;
                KHOANGCAPSYMBOL.directArrow.x2 = _x;
                KHOANGCAPSYMBOL.directArrow.y2 = _y;
                DATA.aArrows.push(KHOANGCAPSYMBOL.directArrow);
                ARROW.update(KHOANGCAPSYMBOL.directArrow.id, 1);
            } else {
                d3.select("g[id=" + ARROW.ID + KHOANGCAPSYMBOL.directArrow.id + "]").remove();
            }
    ...
    remove: function (id) {
        ...
        if (!isMarker) {
            d3.select("g[id=" + KHOANGCAPSYMBOL.ID + id + "]").remove();
    ...
    focus: function (id) {
        if (id != null) {
            $('circle[id^=' + KHOANGCAPSYMBOL.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + KHOANGCAPSYMBOL.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 1);
            $('g[id^=' + KHOANGCAPSYMBOL.ID_REMOVE + id + ']').attr("opacity", 1);
        }
    },

    outFocus: function (id) {
        if (id != null) {
            $('circle[id^=' + KHOANGCAPSYMBOL.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + KHOANGCAPSYMBOL.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 0);
            $('g[id^=' + KHOANGCAPSYMBOL.ID_REMOVE + id + ']').attr("opacity", 0)
        } else {
            $('circle[id^=' + KHOANGCAPSYMBOL.ID_CONNECT_TOP_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + KHOANGCAPSYMBOL.ID_CONNECT_BOTTOM_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('g[id^=' + KHOANGCAPSYMBOL.ID_REMOVE + ']').each(function () {
                $(this).attr("opacity", 0);
            });
        }
    },
    ...
    moveArrows: function (id) {
        ...
        $.each(_attachArrows, function (i, data) {
            ...
            var _gCard = d3.select("g[id=" + KHOANGCAPSYMBOL.ID + id + "]");
    ...
    update: function (id) {
        var _oCurrent = KHOANGCAPSYMBOL.get(id);

        if (typeof _oCurrent !== "undefined" && _oCurrent.type == "khoangcapsymbol") {
            var _deviceName = document.getElementById("khoang-cap-symbol-device-name").value;
            var _description = document.getElementById("khoang-cap-symbol-description").value;
            var _oCard = {
                "id": _oCurrent.id,
...
export default KHOANGCAPSYMBOL;