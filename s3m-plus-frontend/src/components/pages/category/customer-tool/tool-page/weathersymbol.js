...
let WEATHERSYMBOL = {
    ID: "idwtsb-",
    ID_CIRCLE_CONNECT: "wtsblc",
    ID_CONNECT_TOP_MARKER: "wtsblct-",
    ID_CONNECT_LEFT_MARKER: "wtsblcl-",
    ID_CONNECT_RIGHT_MARKER: "wtsblcr-",
    ID_CONNECT_BOTTOM_MARKER: "wtsblcb-",
    ID_REMOVE: "wtsbrm-",
    directArrow: {},

    collect: function () {
        var _deviceName = document.getElementById("weather-symbol-device-name").value;
        var _description = document.getElementById("weather-symbol-description").value;
    ...
    create: function () {
        var _oCard = WEATHERSYMBOL.collect();
        DATA.aCards.push(_oCard);
        WEATHERSYMBOL.draw(_oCard);
    },
    ...
    drawLoad: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            .attr("id", function (d) { return (WEATHERSYMBOL.ID + d.id); })
        ...
        var _path1 = _gCard.datum(card).append("path")
            .attr("d", CONSTANTS.WEATHER_SYMBOL.path1)
    ...
    draw: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            ...
            .on("click", WEATHERSYMBOL.select)
            .call(WEATHERSYMBOL.drag);
        ...
        var _cardTopMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return WEATHERSYMBOL.ID_CONNECT_TOP_MARKER + d.id; })
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", WEATHERSYMBOL.ID_CONNECT_TOP_MARKER)
            .attr("fill", "#237ba0")
            .call(WEATHERSYMBOL.connect);

        var _cardLeftMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return WEATHERSYMBOL.ID_CONNECT_LEFT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 1.5)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", WEATHERSYMBOL.ID_CONNECT_LEFT_MARKER)
            .attr("fill", "#237ba0")
            .call(WEATHERSYMBOL.connect);

        var _cardRightMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return WEATHERSYMBOL.ID_CONNECT_RIGHT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 64.5)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", WEATHERSYMBOL.ID_CONNECT_RIGHT_MARKER)
            .attr("fill", "#237ba0")
            .call(WEATHERSYMBOL.connect);

        var _cardBottomMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return WEATHERSYMBOL.ID_CONNECT_BOTTOM_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 33)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", WEATHERSYMBOL.ID_CONNECT_BOTTOM_MARKER)
            .attr("fill", "#237ba0")
            .call(WEATHERSYMBOL.connect);

        var _gX = _gCard.datum(card).append("g")
            .attr("id", function (d) { return (WEATHERSYMBOL.ID_REMOVE + d.id); })
            .attr("transform", "translate(-20,-30)")
            .attr("opacity", "1")
            .attr("cursor", "pointer")
            .on("click", WEATHERSYMBOL.delete);
    ...
    drag: d3.drag()
        .on("start", function (i, d) {
            ...
            var _oCard = WEATHERSYMBOL.get(d.id);
            document.getElementById("weather-symbol-device-name").value = _oCard.deviceName;
            document.getElementById("weather-symbol-description").value = _oCard.description;
        ...
        .on("drag", function (i, d) {
            var _gCard = d3.select("g[id=" + WEATHERSYMBOL.ID + d.id + "]");
            var _circleTopMaker = d3.select("circle[id=" + WEATHERSYMBOL.ID_CONNECT_TOP_MARKER + d.id + "]");
            var _circleLeftMaker = d3.select("circle[id=" + WEATHERSYMBOL.ID_CONNECT_LEFT_MARKER + d.id + "]");
            var _circleRightMaker = d3.select("circle[id=" + WEATHERSYMBOL.ID_CONNECT_RIGHT_MARKER + d.id + "]");
            var _circleBottomMaker = d3.select("circle[id=" + WEATHERSYMBOL.ID_CONNECT_BOTTOM_MARKER + d.id + "]");

            d3.select(this).attr("transform", "translate(" + (d.x) + "," + (d.y) + ")");
            d.x = i.x;
            d.y = i.y;
            WEATHERSYMBOL.moveArrows(d.id);
            ...
            if (g_connect.cardId == "") {
                d3.selectAll("circle[id^=" + METER.ID_CIRCLE_CONNECT + "]").each(function (c) {
                    ...
                    if (_isOverlapT || _isOverlapL || _isOverlapR || _isOverlapB) {
                        ...
                        if (_isOverlapT) {
                            g_connect.circleFrom = WEATHERSYMBOL.ID_CONNECT_TOP_MARKER;
                        } else if (_isOverlapL) {
                            g_connect.circleFrom = WEATHERSYMBOL.ID_CONNECT_LEFT_MARKER;
                        } else if (_isOverlapR) {
                            g_connect.circleFrom = WEATHERSYMBOL.ID_CONNECT_RIGHT_MARKER;
                        } else if (_isOverlapB) {
                            g_connect.circleFrom = WEATHERSYMBOL.ID_CONNECT_BOTTOM_MARKER;
                        };
                    };
                });
    ...
    connect: d3.drag()
        .on("start", function (i, d) {
            ...
            var _gArrow = g_svg.datum(WEATHERSYMBOL.directArrow).append('g')
            ...
            var _arrowLine = _gArrow.datum(WEATHERSYMBOL.directArrow).append("line")
            ...
            var _arrowStartPoint = _gArrow.datum(WEATHERSYMBOL.directArrow).append('circle')
            ...
            var _arrowEndPoint = _gArrow.datum(WEATHERSYMBOL.directArrow).append('circle')
        ...
        .on("drag", function (event, d) {
            ...
            var _gArrow = d3.select("g[id=" + ARROW.ID + WEATHERSYMBOL.directArrow.id + "]");
            var _arrowLine = d3.select("line[id=" + ARROW.ID_LINE + WEATHERSYMBOL.directArrow.id + "]");
            var _arrowEndPoint = d3.select("circle[id=" + ARROW.ID_LINE_POINT_END + WEATHERSYMBOL.directArrow.id + "]");
        ...
        .on("end", function (i, d) {
            ...
            if (g_connect.attachFlag && g_connect.id != "") {
                ...
                WEATHERSYMBOL.directArrow.to = g_connect.id;
                WEATHERSYMBOL.directArrow.toMarker = g_connect.attachMarker;
                WEATHERSYMBOL.directArrow.x2 = _x;
                WEATHERSYMBOL.directArrow.y2 = _y;
                DATA.aArrows.push(WEATHERSYMBOL.directArrow);
                ARROW.update(WEATHERSYMBOL.directArrow.id, 1);
            } else {
                d3.select("g[id=" + ARROW.ID + WEATHERSYMBOL.directArrow.id + "]").remove();
            }
    ...
    remove: function (id) {
        ...
        if (!isMarker) {
            d3.select("g[id=" + WEATHERSYMBOL.ID + id + "]").remove();
    ...
    focus: function (id) {
        if (id != null) {
            $('circle[id^=' + WEATHERSYMBOL.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + WEATHERSYMBOL.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + WEATHERSYMBOL.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + WEATHERSYMBOL.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 1);
            $('g[id^=' + WEATHERSYMBOL.ID_REMOVE + id + ']').attr("opacity", 1);
        }
    },

    outFocus: function (id) {
        if (id != null) {
            $('circle[id^=' + WEATHERSYMBOL.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + WEATHERSYMBOL.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + WEATHERSYMBOL.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + WEATHERSYMBOL.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 0);
            $('g[id^=' + WEATHERSYMBOL.ID_REMOVE + id + ']').attr("opacity", 0);
        } else {
            $('circle[id^=' + WEATHERSYMBOL.ID_CONNECT_TOP_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + WEATHERSYMBOL.ID_CONNECT_LEFT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + WEATHERSYMBOL.ID_CONNECT_RIGHT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + WEATHERSYMBOL.ID_CONNECT_BOTTOM_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('g[id^=' + WEATHERSYMBOL.ID_REMOVE + ']').each(function () {
                $(this).attr("opacity", 0);
            });
        }
    },
    ...
    moveArrows: function (id) {
        ...
        $.each(_attachArrows, function (i, data) {
            ...
            var _gCard = d3.select("g[id=" + WEATHERSYMBOL.ID + id + "]");
    ...
    update: function (id) {
        var _oCurrent = WEATHERSYMBOL.get(id);

        if (typeof _oCurrent !== "undefined" && _oCurrent.type == "weathersymbol") {
            var _deviceName = document.getElementById("weather-symbol-device-name").value;
            var _description = document.getElementById("weather-symbol-description").value;
            var _oCard = {
                "id": _oCurrent.id,
