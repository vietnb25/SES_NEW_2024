...
let KHOANGCAP = {
    ID: "idkhc-",
    ID_CIRCLE_CONNECT: "khcc",
    ID_CONNECT_TOP_MARKER: "khcct-",
    ID_CONNECT_LEFT_MARKER: "khccl-",
    ID_CONNECT_RIGHT_MARKER: "khccr-",
    ID_CONNECT_BOTTOM_MARKER: "khccb-",

    ID_STATUS_KHOANGCAP: "iskhc-",
    ID_PATH_1_KHOANGCAP: "ip1khc-",
    ID_PATH_2_KHOANGCAP: "ip2khc-",
    ID_REMOVE: "irmkhc-",
    ID_DETAIL: "idtkhc-",
    ID_FOCUS: "ifkhc-",

    KHOANGCAP_T_TEXT: "T (°C)",
    KHOANGCAP_H_TEXT: "H (%)",
    KHOANGCAP_P_TEXT: "PD",

    ID_KHOANGCAP_VALUE_T: "ivtkhc-",
    ID_KHOANGCAP_VALUE_H: "ivhkhc-",
    ID_KHOANGCAP_VALUE_P: "ivpkhc-",
    ...
    collect: function () {
        var _tagName = document.getElementById("khoang-cap-tag-name").value;
        var _deviceName = document.getElementById("khoang-cap-device-name").value;
        var _description = document.getElementById("khoang-cap-description").value;
        var _deviceCaculator = Number($("input[name=khoang_cap_caculator]").filter(":checked").val());
    ...
    create: function () {
        var _oCard = KHOANGCAP.collect();
        DATA.aCards.push(_oCard);
        KHOANGCAP.draw(_oCard);
    },
    ...
    drawLoad: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            .attr("id", function (d) { return (KHOANGCAP.ID + d.id); })
        ...
        var _path1 = _gCard.datum(card).append("path")
            .attr("id", function (d) { return (KHOANGCAP.ID_PATH_1_KHOANGCAP + d.deviceId); })
            .attr("d", CONSTANTS.KHOANGCAP.path1)
        ...
        var _textT = _gCard.datum(card).append("text")
            ...
            .attr("text-anchor", "middle")
            .attr("font-size", "15")
            .text(KHOANGCAP.KHOANGCAP_T_TEXT);
        ...
        var _textH = _gCard.datum(card).append("text")
            ...
            .attr("text-anchor", "middle")
            .attr("font-size", "15")
            .text(KHOANGCAP.KHOANGCAP_H_TEXT);
        ...
        var _textP = _gCard.datum(card).append("text")
            ...
            .attr("text-anchor", "middle")
            .attr("font-size", "15")
            .text(KHOANGCAP.KHOANGCAP_P_TEXT);
        ...
        var _rect = _gCard.datum(card).append("rect")
            .attr("id", function (d) { return (KHOANGCAP.ID_FOCUS + d.id); })
            .attr("x", "-30")
            .attr("y", "-30")
            .attr("width", "238")
            .attr("height", "238")
            .attr("fill", "transparent")
            .on("click", KHOANGCAP.detail);

        if (card.caculator == 1) {
            var _gCaculator = _gCard.datum(card).append('g')
                .attr("transform", "translate(157,132)");
    ...
    draw: function (card) {
        ...
        var _cardTopMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return KHOANGCAP.ID_CONNECT_TOP_MARKER + d.id; })
            ...
            .attr("opacity", 0.5)
            .attr("cursor", "pointer")
            .attr("marker", KHOANGCAP.ID_CONNECT_TOP_MARKER)
            .attr("fill", "#237ba0")
            .call(KHOANGCAP.connect);

        var _cardBottomMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return KHOANGCAP.ID_CONNECT_BOTTOM_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 52.9)
            ...
            .attr("opacity", 0.5)
            .attr("cursor", "pointer")
            .attr("marker", KHOANGCAP.ID_CONNECT_BOTTOM_MARKER)
            .attr("fill", "#237ba0")
            .call(KHOANGCAP.connect);

        var _gX = _gCard.datum(card).append("g")
            .attr("id", function (d) { return (KHOANGCAP.ID_REMOVE + d.id); })
            .attr("transform", "translate(-20,-40)")
            .attr("opacity", "1")
            .attr("cursor", "pointer")
            .on("click", KHOANGCAP.delete);
    ...
    drag: d3.drag()
        .on("start", function (i, d) {
            ...
            var _oCard = KHOANGCAP.get(d.id);
            document.getElementById("khoang-cap-device-name").value = _oCard.deviceName;
            document.getElementById("khoang-cap-tag-name").value = _oCard.tagName;
            document.getElementById("khoang-cap-description").value = _oCard.description;

            if (Number(_oCard.caculator) == 1) {
                document.getElementById("khoang-cap-caculator-1").checked = true;
            } else {
                document.getElementById("khoang-cap-caculator-0").checked = true;
            };
        ...
        .on("drag", function (i, d) {
            var _gCard = d3.select("g[id=" + KHOANGCAP.ID + d.id + "]");
            var _circleTopMaker = d3.select("circle[id=" + KHOANGCAP.ID_CONNECT_TOP_MARKER + d.id + "]");
            var _circleBottomMaker = d3.select("circle[id=" + KHOANGCAP.ID_CONNECT_BOTTOM_MARKER + d.id + "]");

            d3.select(this).attr("transform", "translate(" + (d.x) + "," + (d.y) + ")");
            d.x = i.x;
            d.y = i.y;
            KHOANGCAP.moveArrows(d.id);
            ...
            if (g_connect.cardId == "") {
                d3.selectAll("circle[id^=" + METER.ID_CIRCLE_CONNECT + "]").each(function (c) {
                    ...
                    if (_isOverlapT || _isOverlapB) {
                        ...
                        if (_isOverlapT) {
                            g_connect.circleFrom = KHOANGCAP.ID_CONNECT_TOP_MARKER;
                        } else if (_isOverlapB) {
                            g_connect.circleFrom = KHOANGCAP.ID_CONNECT_BOTTOM_MARKER;
                        };
                    };
                });
    ...
    connect: d3.drag()
        .on("start", function (i, d) {
            ...
            var _gArrow = g_svg.datum(KHOANGCAP.directArrow).append('g')
            ...
            var _arrowLine = _gArrow.datum(KHOANGCAP.directArrow).append("line")
            ...
            var _arrowStartPoint = _gArrow.datum(KHOANGCAP.directArrow).append('circle')
            ...
            var _arrowEndPoint = _gArrow.datum(KHOANGCAP.directArrow).append('circle')
        ...
        .on("drag", function (event, d) {
            ...
            var _gArrow = d3.select("g[id=" + ARROW.ID + KHOANGCAP.directArrow.id + "]");
            var _arrowLine = d3.select("line[id=" + ARROW.ID_LINE + KHOANGCAP.directArrow.id + "]");
            var _arrowEndPoint = d3.select("circle[id=" + ARROW.ID_LINE_POINT_END + KHOANGCAP.directArrow.id + "]");
        ...
        .on("end", function (i, d) {
            ...
            if (g_connect.attachFlag && g_connect.id != "") {
                ...
                KHOANGCAP.directArrow.to = g_connect.id;
                KHOANGCAP.directArrow.toMarker = g_connect.attachMarker;
                KHOANGCAP.directArrow.x2 = _x;
                KHOANGCAP.directArrow.y2 = _y;
                DATA.aArrows.push(KHOANGCAP.directArrow);
                ARROW.update(KHOANGCAP.directArrow.id, 1);
            } else {
                d3.select("g[id=" + ARROW.ID + KHOANGCAP.directArrow.id + "]").remove();
            }
    ...
    remove: function (id) {
        ...
        if (!isMarker) {
            ...
            d3.select("g[id=" + KHOANGCAP.ID + id + "]").remove();
    ...
    focus: function (id) {
        if (id != null) {
            $('circle[id^=' + KHOANGCAP.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + KHOANGCAP.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 1);
            $('g[id^=' + KHOANGCAP.ID_REMOVE + id + ']').attr("opacity", 1);
        }
    },

    outFocus: function (id) {
        if (id != null) {
            $('circle[id^=' + KHOANGCAP.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + KHOANGCAP.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 0);
            $('g[id^=' + KHOANGCAP.ID_REMOVE + id + ']').attr("opacity", 0)
        } else {
            $('circle[id^=' + KHOANGCAP.ID_CONNECT_TOP_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + KHOANGCAP.ID_CONNECT_BOTTOM_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('g[id^=' + KHOANGCAP.ID_REMOVE + ']').each(function () {
                $(this).attr("opacity", 0);
            });
        }
    },
    ...
    moveArrows: function (id) {
        ...
        $.each(_attachArrows, function (i, data) {
            ...
            var _gCard = d3.select("g[id=" + KHOANGCAP.ID + id + "]");
    ...
    update: function (id) {
        var _oCurrent = KHOANGCAP.get(id);

        if (typeof _oCurrent !== "undefined" && _oCurrent.type == "khoangcap") {
            var _deviceCaculator = Number($("input[name=khoang_cap_caculator]").filter(":checked").val());
            var _tagName = document.getElementById("khoang-cap-tag-name").value;
            var _description = document.getElementById("khoang-cap-description").value;
            var _oCard = {
