...
let MBA = {
    ID: "gmba-",
    ID_CIRCLE_CONNECT: "mbac",
    ID_CONNECT_TOP_MARKER: "mbact-",
    ID_CONNECT_BOTTOM_MARKER: "mbacb-",
    ID_REMOVE: "mbarm",
    directArrow: {},

    collect: function () {
        var _deviceName = document.getElementById("mba-device-name").value;
        var _description = document.getElementById("mba-description").value;
    ...
    drawLoad: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            .attr("id", function (d) { return (MBA.ID + d.id); })
    ...
    draw: function (card) {
        ...
        var _cardTopMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return MBA.ID_CONNECT_TOP_MARKER + d.id; })
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", MBA.ID_CONNECT_TOP_MARKER)
            .attr("fill", "#237ba0")
            .call(MBA.connect);

        var _cardBottomMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return MBA.ID_CONNECT_BOTTOM_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 26)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", MBA.ID_CONNECT_BOTTOM_MARKER)
            .attr("fill", "#237ba0")
            .call(MBA.connect);

        var _gX = _gCard.datum(card).append("g")
            .attr("id", function (d) { return (MBA.ID_REMOVE + d.id); })
    ...
    drag: d3.drag()
        .on("start", function (i, d) {
            ...
            var _oCard = MBA.get(d.id);
            document.getElementById("mba-device-name").value = _oCard.deviceName;
            document.getElementById("mba-description").value = _oCard.description;
        ...
        .on("drag", function (i, d) {
            var _gCard = d3.select("g[id=" + MBA.ID + d.id + "]");
            var _circleTopMaker = d3.select("circle[id=" + MBA.ID_CONNECT_TOP_MARKER + d.id + "]");
            var _circleBottomMaker = d3.select("circle[id=" + MBA.ID_CONNECT_BOTTOM_MARKER + d.id + "]");
            ...
            if (g_connect.cardId == "") {
                d3.selectAll("circle[id^=" + METER.ID_CIRCLE_CONNECT + "]").each(function (c) {
                    ...
                    if (_isOverlapT || _isOverlapB) {
                        ...
                        if (_isOverlapT) {
                            g_connect.circleFrom = MBA.ID_CONNECT_TOP_MARKER;
                        } else if (_isOverlapB) {
                            g_connect.circleFrom = MBA.ID_CONNECT_BOTTOM_MARKER;
                        };
                    };
                });
    ...
    connect: d3.drag()
        .on("start", function (i, d) {
            ...
            var _gArrow = g_svg.datum(MBA.directArrow).append('g')
            ...
            var _arrowLine = _gArrow.datum(MBA.directArrow).append("line")
            ...
            var _arrowStartPoint = _gArrow.datum(MBA.directArrow).append('circle')
            ...
            var _arrowEndPoint = _gArrow.datum(MBA.directArrow).append('circle')
        ...
        .on("drag", function (event, d) {
            ...
            var _gArrow = d3.select("g[id=" + ARROW.ID + MBA.directArrow.id + "]");
            var _arrowLine = d3.select("line[id=" + ARROW.ID_LINE + MBA.directArrow.id + "]");
            var _arrowEndPoint = d3.select("circle[id=" + ARROW.ID_LINE_POINT_END + MBA.directArrow.id + "]");
        ...
        .on("end", function (i, d) {
            ...
            if (g_connect.attachFlag && g_connect.id != "") {
                ...
                MBA.directArrow.to = g_connect.id;
                MBA.directArrow.toMarker = g_connect.attachMarker;
                MBA.directArrow.x2 = _x;
                MBA.directArrow.y2 = _y;
                DATA.aArrows.push(MBA.directArrow);
                ARROW.update(MBA.directArrow.id, 1);
            } else {
                d3.select("g[id=" + ARROW.ID + MBA.directArrow.id + "]").remove();
            }
    ...
    remove: function (id) {
        ...
        if (!isMarker) {
            d3.select("g[id=" + MBA.ID + id + "]").remove();
    ...
    focus: function (id) {
        if (id != null) {
            $('circle[id^=' + MBA.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + MBA.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 1);
            $('g[id^=' + MBA.ID_REMOVE + id + ']').attr("opacity", 1);
        }
    },

    outFocus: function (id) {
        if (id != null) {
            $('circle[id^=' + MBA.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + MBA.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 0);
            $('g[id^=' + MBA.ID_REMOVE + id + ']').attr("opacity", 0);
        } else {
            $('circle[id^=' + MBA.ID_CONNECT_TOP_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + MBA.ID_CONNECT_BOTTOM_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('g[id^=' + MBA.ID_REMOVE + ']').each(function () {
                $(this).attr("opacity", 0);
            });
        }
    },
    ...
    moveArrows: function (id) {
        ...
        $.each(_attachArrows, function (i, data) {
            ...
            var _gCard = d3.select("g[id=" + MBA.ID + id + "]");
    ...
    update: function (id) {
        ...
        var _oCurrent = MBA.get(id);

        if (typeof _oCurrent !== "undefined" && _oCurrent.type == "mba") {
            var _deviceName = document.getElementById("mba-device-name").value;
            var _description = document.getElementById("mba-description").value;
            var _oCard = {
                "id": _oCurrent.id,
