...
let STMV = {
    ID: "idgmt-",
    ID_CIRCLE_CONNECT: "gmtc",
    ID_CONNECT_TOP_MARKER: "gmtct-",
    ID_CONNECT_BOTTOM_MARKER: "gmtcb-",
    ID_STATUS_SGMV: "sigmt-",
    ID_PATH_SGMV: "pigmt-",
    ID_REMOVE: "gmtrm",
    ID_DETAIL: "idtgmt-",
    ID_FOCUS: "ifmgt-",
    ...
    ID_STMV_VALUE_U: "igtu-",
    ID_STMV_VALUE_I: "igti-",
    ID_STMV_VALUE_P: "igtp-",
    ID_STMV_VALUE_T: "igtt-",

    directArrow: {},

    collect: function () {
        var _tagName = document.getElementById("stmv-tag-name").value;
        var _deviceName = document.getElementById("stmv-device-name").value;
        var _description = document.getElementById("stmv-description").value;
        var _deviceCaculator = Number($("input[name=stmv_caculator]").filter(":checked").val());
    ...
    drawLoad: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            .attr("id", function (d) { return (STMV.ID + d.id); })
        ...
        var _path1 = _gCard.datum(card).append("path")
            .attr("id", function (d) { return (STMV.ID_PATH_SGMV + d.deviceId); })
            .attr("d", CONSTANTS.STMV.path1)
        ...
        var _textT = _gCard.datum(card).append("text")
            ...
            .attr("text-anchor", "middle")
            .attr("font-size", "15")
            .text(STMV.SGMV_T_TEXT);

        var _valueU = _gCard.datum(card).append("text")
            .attr("id", function (d) { return (STMV.ID_STMV_VALUE_U + d.deviceId); })
            .attr("x", "197")
            .attr("y", "86")
            .attr("fill", "black")
            .attr("dominant-baseline", "middle")
        ...
        var _valueI = _gCard.datum(card).append("text")
            .attr("id", function (d) { return (STMV.ID_STMV_VALUE_I + d.deviceId); })
            .attr("x", "197")
            .attr("y", "116")
            .attr("fill", "black")
            .attr("dominant-baseline", "middle")
        ...
        var _valueP = _gCard.datum(card).append("text")
            .attr("id", function (d) { return (STMV.ID_STMV_VALUE_P + d.deviceId); })
            .attr("x", "197")
            .attr("y", "146")
            .attr("fill", "black")
            .attr("dominant-baseline", "middle")
        ...
        var _valueT = _gCard.datum(card).append("text")
            .attr("id", function (d) { return (STMV.ID_STMV_VALUE_T + d.deviceId); })
            .attr("x", "197")
            .attr("y", "172")
            .attr("fill", "black")
            .attr("dominant-baseline", "middle")
        ...
        var _rect = _gCard.datum(card).append("rect")
            .attr("id", function (d) { return (STMV.ID_FOCUS + d.id); })
            .attr("x", "-20")
            .attr("y", "-20")
            .attr("width", "280")
            .attr("height", "240")
    ...
    draw: function (card) {
        ...
        var _cardTopMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return STMV.ID_CONNECT_TOP_MARKER + d.id; })
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", STMV.ID_CONNECT_TOP_MARKER)
            .attr("fill", "#237ba0")
            .call(STMV.connect);

        var _cardBottomMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return STMV.ID_CONNECT_BOTTOM_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 26.5)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", STMV.ID_CONNECT_BOTTOM_MARKER)
            .attr("fill", "#237ba0")
            .call(STMV.connect);

        var _gX = _gCard.datum(card).append("g")
            .attr("id", function (d) { return (STMV.ID_REMOVE + d.id); })
            .attr("transform", "translate(-20,-40)")
            .attr("opacity", "0")
            .attr("cursor", "pointer")
            .on("click", STMV.delete);
    ...
    drag: d3.drag()
        .on("start", function (i, d) {
            ...
            var _oCard = STMV.get(d.id);
            document.getElementById("stmv-device-name").value = _oCard.deviceName;
            document.getElementById("stmv-tag-name").value = _oCard.tagName;
            document.getElementById("stmv-description").value = _oCard.description;

            if (Number(_oCard.caculator) == 1) {
                document.getElementById("stmv-caculator-1").checked = true;
            } else {
                document.getElementById("stmv-caculator-0").checked = true;
            };
        ...
        .on("drag", function (i, d) {
            var _gCard = d3.select("g[id=" + STMV.ID + d.id + "]");
            var _circleTopMaker = d3.select("circle[id=" + STMV.ID_CONNECT_TOP_MARKER + d.id + "]");
            var _circleBottomMaker = d3.select("circle[id=" + STMV.ID_CONNECT_BOTTOM_MARKER + d.id + "]");

            d3.select(this).attr("transform", "translate(" + (d.x) + "," + (d.y) + ")");
            d.x = i.x;
            d.y = i.y;
            STMV.moveArrows(d.id);
            ...
            if (g_connect.cardId == "") {
                d3.selectAll("circle[id^=" + METER.ID_CIRCLE_CONNECT + "]").each(function (c) {
                    ...
                    if (_isOverlapT || _isOverlapB) {
                        ...
                        if (_isOverlapT) {
                            g_connect.circleFrom = STMV.ID_CONNECT_TOP_MARKER;
                        } else if (_isOverlapB) {
                            g_connect.circleFrom = STMV.ID_CONNECT_BOTTOM_MARKER;
                        };
                    };
                });
    ...
    connect: d3.drag()
        .on("start", function (i, d) {
            ...
            var _gArrow = g_svg.datum(STMV.directArrow).append('g')
            ...
            var _arrowLine = _gArrow.datum(STMV.directArrow).append("line")
            ...
            var _arrowStartPoint = _gArrow.datum(STMV.directArrow).append('circle')
            ...
            var _arrowEndPoint = _gArrow.datum(STMV.directArrow).append('circle')
        ...
        .on("drag", function (event, d) {
            ...
            var _gArrow = d3.select("g[id=" + ARROW.ID + STMV.directArrow.id + "]");
            var _arrowLine = d3.select("line[id=" + ARROW.ID_LINE + STMV.directArrow.id + "]");
            var _arrowEndPoint = d3.select("circle[id=" + ARROW.ID_LINE_POINT_END + STMV.directArrow.id + "]");
        ...
        .on("end", function (i, d) {
            ...
            if (g_connect.attachFlag && g_connect.id != "") {
                ...
                STMV.directArrow.to = g_connect.id;
                STMV.directArrow.toMarker = g_connect.attachMarker;
                STMV.directArrow.x2 = _x;
                STMV.directArrow.y2 = _y;
                DATA.aArrows.push(STMV.directArrow);
                ARROW.update(STMV.directArrow.id, 1);
            } else {
                d3.select("g[id=" + ARROW.ID + STMV.directArrow.id + "]").remove();
            }
    ...
    remove: function (id) {
        ...
        if (!isMarker) {
            var stmvIndex = DATA.aCards.findIndex(x => x.id === id);
            $("#stmvIds").append(new Option(DATA.aCards[stmvIndex].deviceName, DATA.aCards[stmvIndex].deviceId));

            d3.select("g[id=" + STMV.ID + id + "]").remove();
    ...
    focus: function (id) {
        if (id != null) {
            $('circle[id^=' + STMV.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + STMV.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 1);
            $('g[id^=' + STMV.ID_REMOVE + id + ']').attr("opacity", 1);
        }
    },

    outFocus: function (id) {
        if (id != null) {
            $('circle[id^=' + STMV.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + STMV.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 0);
            $('g[id^=' + STMV.ID_REMOVE + id + ']').attr("opacity", 0)
        } else {
            $('circle[id^=' + STMV.ID_CONNECT_TOP_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + STMV.ID_CONNECT_BOTTOM_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('g[id^=' + STMV.ID_REMOVE + ']').each(function () {
                $(this).attr("opacity", 0);
            });
        }
    },
    ...
    moveArrows: function (id) {
        ...
        $.each(_attachArrows, function (i, data) {
            ...
            var _gCard = d3.select("g[id=" + STMV.ID + id + "]");
    ...
    update: function (id) {
        ...
        var _oCurrent = STMV.get(id);

        if (typeof _oCurrent !== "undefined" && _oCurrent.type == "stmv") {
            var _deviceCaculator = Number($("input[name=stmv_caculator]").filter(":checked").val());
            var _tagName = document.getElementById("stmv-tag-name").value;
            var _description = document.getElementById("stmv-description").value;
            var _oCard = {
