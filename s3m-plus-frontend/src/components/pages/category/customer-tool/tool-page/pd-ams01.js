...
let PD_AMS01 = {
    ID: "ams01-",
    ID_CIRCLE_CONNECT: "ams01_c",
    ID_CONNECT_TOP_MARKER: "ams01_ct-",
    ID_CONNECT_BOTTOM_MARKER: "ams01_cb-",
    ID_CONNECT_LEFT_MARKER: "ams01_cl-",
    ID_CONNECT_RIGHT_MARKER: "ams01_cr-",
    ID_REMOVE: "ams01_rm",
    ID_FOCUS: "ams01_fc",
    ID_STATUS: "ams01_status",
    ...
    create: function () {
        var _oCard = PD_AMS01.collect();
        DATA.aCards.push(_oCard);
        PD_AMS01.draw(_oCard);
    },
    ...
    drawLoad: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            .attr("id", function (d) { return (PD_AMS01.ID + d.id); })
        ...
        var _deviceIndicator = _gCard.datum(card).append("text")
            .attr("id", function (d) { return (PD_AMS01.INDICATOR_TEXT + d.deviceId); })
            .attr("x", "50")
            .attr("y", "35")
            .attr("fill", "black")
            .attr("font-size", "12");
        ...
        var _rect = _gCard.datum(card).append("rect")
            .attr("id", function (d) { return (PD_AMS01.ID_FOCUS + d.id) })
            .attr("x", "-5")
            .attr("y", "-5")
            .attr("width", (PD_AMS01.DEFAULT_WIDTH + 10))
            .attr("height", (PD_AMS01.DEFAULT_HEIGHT + 10))
            .attr("fill", "transparent");

        var _rectBackground = _gCard.datum(card).append("rect")
            .attr("id", function (d) { return (PD_AMS01.ID_STATUS + d.deviceId) })
            .attr("x", "5")
            .attr("y", "5")
            .attr("width", (PD_AMS01.DEFAULT_WIDTH - 10))
            .attr("height", (PD_AMS01.DEFAULT_HEIGHT - 10))
        ...
        var _image = _gCard.datum(card).append("image")
            .attr("href", "/resources/image/components/cb-phong-dien/cb_phong_dien_ams01.svg")
            .attr("width", PD_AMS01.DEFAULT_WIDTH)
            .attr("height", PD_AMS01.DEFAULT_HEIGHT)
            .attr("cursor", "pointer")
            .on("click", PD_AMS01.detail);

        var _cardTopMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return PD_AMS01.ID_CONNECT_TOP_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", (PD_AMS01.DEFAULT_WIDTH / 2))
            .attr("cy", 0)
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", PD_AMS01.ID_CONNECT_TOP_MARKER)
            .attr("fill", "#237ba0")

        var _cardBottomMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return PD_AMS01.ID_CONNECT_BOTTOM_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", (PD_AMS01.DEFAULT_WIDTH / 2))
            .attr("cy", PD_AMS01.DEFAULT_HEIGHT)
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", PD_AMS01.ID_CONNECT_BOTTOM_MARKER)
            .attr("fill", "#237ba0")

        var _cardLeftMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return PD_AMS01.ID_CONNECT_LEFT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 0)
            .attr("cy", (PD_AMS01.DEFAULT_HEIGHT / 2))
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", PD_AMS01.ID_CONNECT_LEFT_MARKER)
            .attr("fill", "#237ba0")

        var _cardRightMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return PD_AMS01.ID_CONNECT_RIGHT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", PD_AMS01.DEFAULT_WIDTH)
            .attr("cy", (PD_AMS01.DEFAULT_HEIGHT / 2))
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", PD_AMS01.ID_CONNECT_RIGHT_MARKER)
            .attr("fill", "#237ba0")
    ...
    draw: function (card) {
        ...
        var _rectBackground = _gCard.datum(card).append("rect")
            .attr("x", "2")
            .attr("y", "2")
            .attr("width", (PD_AMS01.DEFAULT_WIDTH - 4))
            .attr("height", (PD_AMS01.DEFAULT_HEIGHT - 4))
            .attr("fill", "transparent")
            .attr("cursor", "pointer")
        ...
        var _gX = _gCard.datum(card).append("g")
            .attr("id", function (d) { return (PD_AMS01.ID_REMOVE + d.id); })
            .attr("transform", "translate(-25,-30)")
            .attr("opacity", "0")
            .attr("cursor", "pointer")
            .on("click", PD_AMS01.delete);
    ...
    drag: d3.drag()
        .on("start", function (i, d) {
            ...
            deviceAlreadyList.forEach(element => {
                if (element.deviceId != "") {
                    $("#ams01-device" + " option[value='" + element.deviceId + "']").each(function () {
                        $(this).remove();
                    });
                }
            });

            var selectElement = document.querySelectorAll('[id=' + "ams01-device" + ']');
            ...
            if (typeof deviceIdOption === "undefined" && deviceIdOption != "") {
                $("#ams01-device").append(new Option(_oCard.deviceName, _oCard.deviceId));
            }
            document.getElementById("ams01-device").value = _oCard.deviceId;
        ...
        .on("drag", function (i, d) {
            var _gCard = d3.select("g[id=" + PD_AMS01.ID + d.id + "]");
            var _circleTopMaker = d3.select("circle[id=" + PD_AMS01.ID_CONNECT_TOP_MARKER + d.id + "]");
            var _circleBottomMaker = d3.select("circle[id=" + PD_AMS01.ID_CONNECT_BOTTOM_MARKER + d.id + "]");
            var _circleLeftMaker = d3.select("circle[id=" + PD_AMS01.ID_CONNECT_LEFT_MARKER + d.id + "]");
            var _circleRightMaker = d3.select("circle[id=" + PD_AMS01.ID_CONNECT_RIGHT_MARKER + d.id + "]");

            d3.select(this).attr("transform", "translate(" + (d.x) + "," + (d.y) + ")");
            d.x = i.x;
            d.y = i.y;
            PD_AMS01.moveArrows(d.id);

            dragAll(d, _gCard);

            if (g_connect.cardId == "") {
                dragConnect(PD_AMS01, d, _circleTopMaker, _circleBottomMaker, _circleLeftMaker, _circleRightMaker);
            };
            ...
            if (g_connect.attachFlag && g_connect.id != "") {
                ...
                switch (g_connect.type) {
                    ...
                    case "pd-ams01":
                        ...
                        break;
                    case "pd-htr02":
                        _cardTo = d3.select("g[id=" + PDHTR02.ID + g_connect.cardId + "]");
                        break;
                };
    ...
    connect: d3.drag()
        .on("start", function (i, d) {
            ...
            var _gArrow = g_svg.datum(PD_AMS01.directArrow).append('g')
            ...
            var _arrowLine = _gArrow.datum(PD_AMS01.directArrow).append("line")
            ...
            var _arrowStartPoint = _gArrow.datum(PD_AMS01.directArrow).append('circle')
            ...
            var _arrowEndPoint = _gArrow.datum(PD_AMS01.directArrow).append('circle')
        ...
        .on("drag", function (event, d) {
            ...
            var _gArrow = d3.select("g[id=" + ARROW.ID + PD_AMS01.directArrow.id + "]");
            var _arrowLine = d3.select("line[id=" + ARROW.ID_LINE + PD_AMS01.directArrow.id + "]");
            var _arrowEndPoint = d3.select("circle[id=" + ARROW.ID_LINE_POINT_END + PD_AMS01.directArrow.id + "]");
        ...
        .on("end", function (i, d) {
            ...
            if (g_connect.attachFlag && g_connect.id != "") {
                ...
                PD_AMS01.directArrow.to = g_connect.id;
                PD_AMS01.directArrow.toMarker = g_connect.attachMarker;
                PD_AMS01.directArrow.x2 = _x;
                PD_AMS01.directArrow.y2 = _y;
                DATA.aArrows.push(PD_AMS01.directArrow);
                ARROW.update(PD_AMS01.directArrow.id, 1);
            } else {
                d3.select("g[id=" + ARROW.ID + PD_AMS01.directArrow.id + "]").remove();
            }
    ...
    remove: function (id) {
        ...
        if (!isMarker) {
            d3.select("g[id=" + PD_AMS01.ID + id + "]").remove();
            ...
            var deviceSelect = document.getElementById("ams01-device")
            deviceSelect.value = "";
        }
    },
    ...
    focus: function (id) {
        if (id != null) {
            $('circle[id^=' + PD_AMS01.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + PD_AMS01.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + PD_AMS01.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + PD_AMS01.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 1);
            $('g[id^=' + PD_AMS01.ID_REMOVE + id + ']').attr("opacity", 1);
        }
    },

    outFocus: function (id) {
        if (id != null) {
            $('circle[id^=' + PD_AMS01.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + PD_AMS01.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + PD_AMS01.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + PD_AMS01.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 0);
            $('g[id^=' + PD_AMS01.ID_REMOVE + id + ']').attr("opacity", 0)
        } else {
            $('circle[id^=' + PD_AMS01.ID_CONNECT_TOP_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + PD_AMS01.ID_CONNECT_LEFT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + PD_AMS01.ID_CONNECT_RIGHT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + PD_AMS01.ID_CONNECT_BOTTOM_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('g[id^=' + PD_AMS01.ID_REMOVE + ']').each(function () {
                $(this).attr("opacity", 0);
            });
        }
    },
    ...
    moveArrows: function (id) {
        ...
        $.each(_attachArrows, function (i, data) {
            ...
            var _gCard = d3.select("g[id=" + PD_AMS01.ID + id + "]");
    ...
    update: function (id) {
        if (id != 0) {
            var _oCurrent = PD_AMS01.get(id);
            if (typeof _oCurrent !== "undefined" && _oCurrent.type == "pd-ams01") {
                ...
                if (_oCard.deviceId != "" && _oCurrent.deviceId != _oCurrent.deviceId && typeof deviceIdOption === "undefined") {
                    $("#ams01-device" + _oCurrent.deviceType).append(new Option(_oCard.deviceName, _oCard.deviceId));
                }
    ...
    delete: function (e) {
        e.stopPropagation();
        PD_AMS01.remove(DATA.selectedID);
    },
