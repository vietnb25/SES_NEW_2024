...
let PANELSYMBOL = {
    ID: "idpanelsb-",
    ID_CIRCLE_CONNECT: "panelsblc",
    ID_CONNECT_TOP_MARKER: "panelsblct-",
    ID_CONNECT_LEFT_MARKER: "panelsblcl-",
    ID_CONNECT_RIGHT_MARKER: "panelsblcr-",
    ID_CONNECT_BOTTOM_MARKER: "panelsblcb-",
    ID_REMOVE: "panelsbrm-",
    directArrow: {},

    collect: function () {
        var _deviceName = document.getElementById("panel-symbol-device-name").value;
        var _description = document.getElementById("panel-symbol-description").value;
    ...
    create: function () {
        var _oCard = PANELSYMBOL.collect();
        DATA.aCards.push(_oCard);
        PANELSYMBOL.draw(_oCard);
    },
    ...
    drawLoad: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            .attr("id", function (d) { return (PANELSYMBOL.ID + d.id); })
        ...
        var _path1 = _gCard.datum(card).append("path")
            .attr("d", CONSTANTS.PANEL_SYMBOL.path1)
    ...
    draw: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            ...
            .on("click", PANELSYMBOL.select)
            .call(PANELSYMBOL.drag);
        ...
        var _cardTopMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return PANELSYMBOL.ID_CONNECT_TOP_MARKER + d.id; })
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", PANELSYMBOL.ID_CONNECT_TOP_MARKER)
            .attr("fill", "#237ba0")
            .call(PANELSYMBOL.connect);

        var _cardLeftMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return PANELSYMBOL.ID_CONNECT_LEFT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 9)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", PANELSYMBOL.ID_CONNECT_LEFT_MARKER)
            .attr("fill", "#237ba0")
            .call(PANELSYMBOL.connect);

        var _cardRightMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return PANELSYMBOL.ID_CONNECT_RIGHT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 78)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", PANELSYMBOL.ID_CONNECT_RIGHT_MARKER)
            .attr("fill", "#237ba0")
            .call(PANELSYMBOL.connect);

        var _cardBottomMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return PANELSYMBOL.ID_CONNECT_BOTTOM_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 44)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", PANELSYMBOL.ID_CONNECT_BOTTOM_MARKER)
            .attr("fill", "#237ba0")
            .call(PANELSYMBOL.connect);

        var _gX = _gCard.datum(card).append("g")
            .attr("id", function (d) { return (PANELSYMBOL.ID_REMOVE + d.id); })
            .attr("transform", "translate(-20,-30)")
            .attr("opacity", "0")
            .attr("cursor", "pointer")
            .on("click", PANELSYMBOL.delete);
    ...
    drag: d3.drag()
        .on("start", function (i, d) {
            ...
            var _oCard = PANELSYMBOL.get(d.id);
            document.getElementById("panel-symbol-device-name").value = _oCard.deviceName;
            document.getElementById("panel-symbol-description").value = _oCard.description;
        ...
        .on("drag", function (i, d) {
            var _gCard = d3.select("g[id=" + PANELSYMBOL.ID + d.id + "]");
            var _circleTopMaker = d3.select("circle[id=" + PANELSYMBOL.ID_CONNECT_TOP_MARKER + d.id + "]");
            var _circleLeftMaker = d3.select("circle[id=" + PANELSYMBOL.ID_CONNECT_LEFT_MARKER + d.id + "]");
            var _circleRightMaker = d3.select("circle[id=" + PANELSYMBOL.ID_CONNECT_RIGHT_MARKER + d.id + "]");
            var _circleBottomMaker = d3.select("circle[id=" + PANELSYMBOL.ID_CONNECT_BOTTOM_MARKER + d.id + "]");

            d3.select(this).attr("transform", "translate(" + (d.x) + "," + (d.y) + ")");
            d.x = i.x;
            d.y = i.y;
            PANELSYMBOL.moveArrows(d.id);
            ...
            if (g_connect.cardId == "") {
                d3.selectAll("circle[id^=" + METER.ID_CIRCLE_CONNECT + "]").each(function (c) {
                    ...
                    if (_isOverlapT || _isOverlapL || _isOverlapR || _isOverlapB) {
                        ...
                        if (_isOverlapT) {
                            g_connect.circleFrom = PANELSYMBOL.ID_CONNECT_TOP_MARKER;
                        } else if (_isOverlapL) {
                            g_connect.circleFrom = PANELSYMBOL.ID_CONNECT_LEFT_MARKER;
                        } else if (_isOverlapR) {
                            g_connect.circleFrom = PANELSYMBOL.ID_CONNECT_RIGHT_MARKER;
                        } else if (_isOverlapB) {
                            g_connect.circleFrom = PANELSYMBOL.ID_CONNECT_BOTTOM_MARKER;
                        };
                    };
                });
    ...
    connect: d3.drag()
        .on("start", function (i, d) {
            ...
            var _gArrow = g_svg.datum(PANELSYMBOL.directArrow).append('g')
            ...
            var _arrowLine = _gArrow.datum(PANELSYMBOL.directArrow).append("line")
            ...
            var _arrowStartPoint = _gArrow.datum(PANELSYMBOL.directArrow).append('circle')
            ...
            var _arrowEndPoint = _gArrow.datum(PANELSYMBOL.directArrow).append('circle')
        ...
        .on("drag", function (event, d) {
            ...
            var _gArrow = d3.select("g[id=" + ARROW.ID + PANELSYMBOL.directArrow.id + "]");
            var _arrowLine = d3.select("line[id=" + ARROW.ID_LINE + PANELSYMBOL.directArrow.id + "]");
            var _arrowEndPoint = d3.select("circle[id=" + ARROW.ID_LINE_POINT_END + PANELSYMBOL.directArrow.id + "]");
        ...
        .on("end", function (i, d) {
            ...
            if (g_connect.attachFlag && g_connect.id != "") {
                ...
                PANELSYMBOL.directArrow.to = g_connect.id;
                PANELSYMBOL.directArrow.toMarker = g_connect.attachMarker;
                PANELSYMBOL.directArrow.x2 = _x;
                PANELSYMBOL.directArrow.y2 = _y;
                DATA.aArrows.push(PANELSYMBOL.directArrow);
                ARROW.update(PANELSYMBOL.directArrow.id, 1);
            } else {
                d3.select("g[id=" + ARROW.ID + PANELSYMBOL.directArrow.id + "]").remove();
            }
    ...
    remove: function (id) {
        ...
        if (!isMarker) {
            d3.select("g[id=" + PANELSYMBOL.ID + id + "]").remove();
    ...
    focus: function (id) {
        if (id != null) {
            $('circle[id^=' + PANELSYMBOL.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + PANELSYMBOL.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + PANELSYMBOL.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + PANELSYMBOL.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 1);
            $('g[id^=' + PANELSYMBOL.ID_REMOVE + id + ']').attr("opacity", 1);
        }
    },

    outFocus: function (id) {
        if (id != null) {
            $('circle[id^=' + PANELSYMBOL.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + PANELSYMBOL.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + PANELSYMBOL.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + PANELSYMBOL.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 0);
            $('g[id^=' + PANELSYMBOL.ID_REMOVE + id + ']').attr("opacity", 0);
        } else {
            $('circle[id^=' + PANELSYMBOL.ID_CONNECT_TOP_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + PANELSYMBOL.ID_CONNECT_LEFT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + PANELSYMBOL.ID_CONNECT_RIGHT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + PANELSYMBOL.ID_CONNECT_BOTTOM_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('g[id^=' + PANELSYMBOL.ID_REMOVE + ']').each(function () {
                $(this).attr("opacity", 0);
            });
        }
    },
    ...
    moveArrows: function (id) {
        ...
        $.each(_attachArrows, function (i, data) {
            ...
            var _gCard = d3.select("g[id=" + PANELSYMBOL.ID + id + "]");
    ...
    update: function (id) {
        ...
        var _oCurrent = PANELSYMBOL.get(id);

        if (typeof _oCurrent !== "undefined" && _oCurrent.type == "panelsymbol") {
            var _deviceName = document.getElementById("panel-symbol-device-name").value;
            var _description = document.getElementById("panel-symbol-description").value;
            var _oCard = {
                "id": _oCurrent.id,
