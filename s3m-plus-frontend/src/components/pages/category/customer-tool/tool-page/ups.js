...
let UPS = {
    ID: "idups-",
    ID_CIRCLE_CONNECT: "upslc",
    ID_CONNECT_TOP_MARKER: "upslct-",
    ID_CONNECT_LEFT_MARKER: "upslcl-",
    ID_CONNECT_RIGHT_MARKER: "upslcr-",
    ID_CONNECT_BOTTOM_MARKER: "upslcb-",
    ID_REMOVE: "upsrm-",
    directArrow: {},

    collect: function () {
        var _deviceName = document.getElementById("ups-device-name").value;
        var _description = document.getElementById("ups-description").value;
    ...
    drawLoad: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            .attr("id", function (d) { return (UPS.ID + d.id); })
    ...
    draw: function (card) {
        ...
        var _cardTopMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return UPS.ID_CONNECT_TOP_MARKER + d.id; })
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", UPS.ID_CONNECT_TOP_MARKER)
            .attr("fill", "#237ba0")
            .call(UPS.connect);

        var _cardLeftMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return UPS.ID_CONNECT_LEFT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 1.5)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", UPS.ID_CONNECT_LEFT_MARKER)
            .attr("fill", "#237ba0")
            .call(UPS.connect);

        var _cardRightMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return UPS.ID_CONNECT_RIGHT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 47.5)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", UPS.ID_CONNECT_RIGHT_MARKER)
            .attr("fill", "#237ba0")
            .call(UPS.connect);

        var _cardBottomMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return UPS.ID_CONNECT_BOTTOM_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 24.5)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", UPS.ID_CONNECT_BOTTOM_MARKER)
            .attr("fill", "#237ba0")
            .call(UPS.connect);

        var _gX = _gCard.datum(card).append("g")
            .attr("id", function (d) { return (UPS.ID_REMOVE + d.id); })
    ...
    drag: d3.drag()
        .on("start", function (i, d) {
            ...
            var _oCard = UPS.get(d.id);
            document.getElementById("ups-device-name").value = _oCard.deviceName;
            document.getElementById("ups-description").value = _oCard.description;
        ...
        .on("drag", function (i, d) {
            var _gCard = d3.select("g[id=" + UPS.ID + d.id + "]");
            var _circleTopMaker = d3.select("circle[id=" + UPS.ID_CONNECT_TOP_MARKER + d.id + "]");
            var _circleLeftMaker = d3.select("circle[id=" + UPS.ID_CONNECT_LEFT_MARKER + d.id + "]");
            var _circleRightMaker = d3.select("circle[id=" + UPS.ID_CONNECT_RIGHT_MARKER + d.id + "]");
            var _circleBottomMaker = d3.select("circle[id=" + UPS.ID_CONNECT_BOTTOM_MARKER + d.id + "]");
            ...
            if (g_connect.cardId == "") {
                d3.selectAll("circle[id^=" + METER.ID_CIRCLE_CONNECT + "]").each(function (c) {
                    ...
                    if (_isOverlapT || _isOverlapL || _isOverlapR || _isOverlapB) {
                        ...
                        if (_isOverlapT) {
                            g_connect.circleFrom = UPS.ID_CONNECT_TOP_MARKER;
                        } else if (_isOverlapL) {
                            g_connect.circleFrom = UPS.ID_CONNECT_LEFT_MARKER;
                        } else if (_isOverlapR) {
                            g_connect.circleFrom = UPS.ID_CONNECT_RIGHT_MARKER;
                        } else if (_isOverlapB) {
                            g_connect.circleFrom = UPS.ID_CONNECT_BOTTOM_MARKER;
                        };
                    };
                });
    ...
    connect: d3.drag()
        .on("start", function (i, d) {
            ...
            var _gArrow = g_svg.datum(UPS.directArrow).append('g')
            ...
            var _arrowLine = _gArrow.datum(UPS.directArrow).append("line")
            ...
            var _arrowStartPoint = _gArrow.datum(UPS.directArrow).append('circle')
            ...
            var _arrowEndPoint = _gArrow.datum(UPS.directArrow).append('circle')
        ...
        .on("drag", function (event, d) {
            ...
            var _gArrow = d3.select("g[id=" + ARROW.ID + UPS.directArrow.id + "]");
            var _arrowLine = d3.select("line[id=" + ARROW.ID_LINE + UPS.directArrow.id + "]");
            var _arrowEndPoint = d3.select("circle[id=" + ARROW.ID_LINE_POINT_END + UPS.directArrow.id + "]");
        ...
        .on("end", function (i, d) {
            ...
            if (g_connect.attachFlag && g_connect.id != "") {
                ...
                UPS.directArrow.to = g_connect.id;
                UPS.directArrow.toMarker = g_connect.attachMarker;
                UPS.directArrow.x2 = _x;
                UPS.directArrow.y2 = _y;
                DATA.aArrows.push(UPS.directArrow);
                ARROW.update(UPS.directArrow.id, 1);
            } else {
                d3.select("g[id=" + ARROW.ID + UPS.directArrow.id + "]").remove();
            }
    ...
    remove: function (id) {
        ...
        if (!isMarker) {
            d3.select("g[id=" + UPS.ID + id + "]").remove();
    ...
    focus: function (id) {
        if (id != null) {
            $('circle[id^=' + UPS.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + UPS.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + UPS.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + UPS.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 1);
            $('g[id^=' + UPS.ID_REMOVE + id + ']').attr("opacity", 1);
        }
    },

    outFocus: function (id) {
        if (id != null) {
            $('circle[id^=' + UPS.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + UPS.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + UPS.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + UPS.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 0);
            $('g[id^=' + UPS.ID_REMOVE + id + ']').attr("opacity", 0);
        } else {
            $('circle[id^=' + UPS.ID_CONNECT_TOP_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + UPS.ID_CONNECT_LEFT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + UPS.ID_CONNECT_RIGHT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + UPS.ID_CONNECT_BOTTOM_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('g[id^=' + UPS.ID_REMOVE + ']').each(function () {
                $(this).attr("opacity", 0);
            });
        }
    },
    ...
    moveArrows: function (id) {
        ...
        $.each(_attachArrows, function (i, data) {
            ...
            var _gCard = d3.select("g[id=" + UPS.ID + id + "]");
    ...
    update: function (id) {
        ...
        var _oCurrent = UPS.get(id);

        if (typeof _oCurrent !== "undefined" && _oCurrent.type == "ups") {
            var _deviceName = document.getElementById("ups-device-name").value;
            var _description = document.getElementById("ups-description").value;
            var _oCard = {
                "id": _oCurrent.id,
