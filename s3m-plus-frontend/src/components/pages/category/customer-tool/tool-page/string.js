...
let STRING = {
    ID: "idstr-",
    ID_CIRCLE_CONNECT: "strc",
    ID_CONNECT_TOP_MARKER: "strct-",
    ID_CONNECT_LEFT_MARKER: "strcl-",
    ID_CONNECT_RIGHT_MARKER: "strcr-",
    ID_CONNECT_BOTTOM_MARKER: "strcb-",
    ID_STATUS_STRING: "isstr-",
    ID_PATH_STRING: "ipstr-",
    ID_REMOVE: "irmstr-",
    ID_DETAIL: "idtstr-",
    ID_FOCUS: "ifstr-",
    ...
    ID_STRING_VALUE_U: "ivustr-",
    ID_STRING_VALUE_I: "ivistr-",
    ID_STRING_VALUE_T: "ivpstr-",

    directArrow: {},

    collect: function () {
        var _tagName = document.getElementById("string-tag-name").value;
        var _description = document.getElementById("string-description").value;
        var _deviceCaculator = Number($("input[name=string_caculator]").filter(":checked").val());
    ...
    drawLoad: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            .attr("id", function (d) { return (STRING.ID + d.id); })
        ...
        var _path1 = _gCard.datum(card).append("path")
            .attr("id", function (d) { return (STRING.ID_PATH_STRING + d.deviceId); })
        ...
        var _statusPath = _gCard.datum(card).append("path")
            .attr("id", function (d) { return (STRING.ID_STATUS_STRING + d.deviceId); })
            .attr("d", "")
        ...
        var _valueU = _gCard.datum(card).append("text")
            .attr("id", function (d) { return (STRING.ID_STRING_VALUE_U + d.deviceId); })
            .attr("x", "55")
            .attr("y", "30")
            .attr("fill", "black")
            .attr("font-size", "12")
        ...
        var _valueI = _gCard.datum(card).append("text")
            .attr("id", function (d) { return (STRING.ID_STRING_VALUE_I + d.deviceId); })
            .attr("x", "55")
            .attr("y", "48")
            .attr("fill", "black")
            .attr("font-size", "12")
        ...
        var _valueT = _gCard.datum(card).append("text")
            .attr("id", function (d) { return (STRING.ID_STRING_VALUE_T + d.deviceId); })
            .attr("x", "55")
            .attr("y", "67")
            .attr("fill", "black")
            .attr("font-size", "12")
        ...
        var _rect = _gCard.datum(card).append("rect")
            .attr("id", function (d) { return (STRING.ID_FOCUS + d.id); })
            .attr("x", "-20")
            .attr("y", "-20")
            .attr("width", "260")
            .attr("height", "150")
        ...
        var _gCaculator = _gCard.datum(card).append('g')
            .attr("transform", "translate(55, -28)");
    ...
    draw: function (card) {
        ...
        var _cardTopMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return STRING.ID_CONNECT_TOP_MARKER + d.id; })
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", STRING.ID_CONNECT_TOP_MARKER)
            .attr("fill", "#237ba0")
            .call(STRING.connect);

        var _cardLeftMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return STRING.ID_CONNECT_LEFT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 1.5)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", STRING.ID_CONNECT_LEFT_MARKER)
            .attr("fill", "#237ba0")
            .call(STRING.connect);

        var _cardRightMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return STRING.ID_CONNECT_RIGHT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 34.5)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", STRING.ID_CONNECT_RIGHT_MARKER)
            .attr("fill", "#237ba0")
            .call(STRING.connect);

        var _cardBottomMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return STRING.ID_CONNECT_BOTTOM_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 18)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", STRING.ID_CONNECT_BOTTOM_MARKER)
            .attr("fill", "#237ba0")
            .call(STRING.connect);

        var _gX = _gCard.datum(card).append("g")
            .attr("id", function (d) { return (STRING.ID_REMOVE + d.id); })
            .attr("transform", "translate(-20,-40)")
            .attr("opacity", "0")
            .attr("cursor", "pointer")
            .on("click", STRING.delete);
    ...
    drag: d3.drag()
        .on("start", function (i, d) {
            ...
            var selectElement = document.querySelectorAll('[id=stringIds]');
            ...
            if (typeof deviceIdOption === "undefined" && deviceIdOption != "") {
                $("#stringIds").append(new Option(_oCard.deviceName, _oCard.deviceId));
            }
            document.getElementById("stringIds").value = _oCard.deviceId;

            if (Number(_oCard.caculator) == 1) {
                document.getElementById("string-caculator-1").checked = true;
            } else {
                document.getElementById("string-caculator-0").checked = true;
            };
        ...
        .on("drag", function (i, d) {
            var _gCard = d3.select("g[id=" + STRING.ID + d.id + "]");
            var _circleTopMaker = d3.select("circle[id=" + STRING.ID_CONNECT_TOP_MARKER + d.id + "]");
            var _circleLeftMaker = d3.select("circle[id=" + STRING.ID_CONNECT_LEFT_MARKER + d.id + "]");
            var _circleRightMaker = d3.select("circle[id=" + STRING.ID_CONNECT_RIGHT_MARKER + d.id + "]");
            var _circleBottomMaker = d3.select("circle[id=" + STRING.ID_CONNECT_BOTTOM_MARKER + d.id + "]");

            d3.select(this).attr("transform", "translate(" + (d.x) + "," + (d.y) + ")");
            d.x = i.x;
            d.y = i.y;
            STRING.moveArrows(d.id);
            ...
            if (g_connect.cardId == "") {
                d3.selectAll("circle[id^=" + METER.ID_CIRCLE_CONNECT + "]").each(function (c) {
                    ...
                    if (_isOverlapT || _isOverlapL || _isOverlapR || _isOverlapB) {
                        ...
                        if (_isOverlapT) {
                            g_connect.circleFrom = STRING.ID_CONNECT_TOP_MARKER;
                        } else if (_isOverlapL) {
                            g_connect.circleFrom = STRING.ID_CONNECT_LEFT_MARKER;
                        } else if (_isOverlapR) {
                            g_connect.circleFrom = STRING.ID_CONNECT_RIGHT_MARKER;
                        } else if (_isOverlapB) {
                            g_connect.circleFrom = STRING.ID_CONNECT_BOTTOM_MARKER;
                        };
                    };
                });
    ...
    connect: d3.drag()
        .on("start", function (i, d) {
            ...
            var _gArrow = g_svg.datum(STRING.directArrow).append('g')
            ...
            var _arrowLine = _gArrow.datum(STRING.directArrow).append("line")
            ...
            var _arrowStartPoint = _gArrow.datum(STRING.directArrow).append('circle')
            ...
            var _arrowEndPoint = _gArrow.datum(STRING.directArrow).append('circle')
        ...
        .on("drag", function (event, d) {
            ...
            var _gArrow = d3.select("g[id=" + ARROW.ID + STRING.directArrow.id + "]");
            var _arrowLine = d3.select("line[id=" + ARROW.ID_LINE + STRING.directArrow.id + "]");
            var _arrowEndPoint = d3.select("circle[id=" + ARROW.ID_LINE_POINT_END + STRING.directArrow.id + "]");
        ...
        .on("end", function (i, d) {
            ...
            if (g_connect.attachFlag && g_connect.id != "") {
                ...
                STRING.directArrow.to = g_connect.id;
                STRING.directArrow.toMarker = g_connect.attachMarker;
                STRING.directArrow.x2 = _x;
                STRING.directArrow.y2 = _y;
                DATA.aArrows.push(STRING.directArrow);
                ARROW.update(STRING.directArrow.id, 1);
            } else {
                d3.select("g[id=" + ARROW.ID + STRING.directArrow.id + "]").remove();
            }
    ...
    remove: function (id) {
        ...
        if (!isMarker) {
            d3.select("g[id=" + STRING.ID + id + "]").remove();
    ...
    focus: function (id) {
        if (id != null) {
            $('circle[id^=' + STRING.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + STRING.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + STRING.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + STRING.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 1);
            $('g[id^=' + STRING.ID_REMOVE + id + ']').attr("opacity", 1);
        }
    },

    outFocus: function (id) {
        if (id != null) {
            $('circle[id^=' + STRING.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + STRING.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + STRING.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + STRING.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 0);
            $('g[id^=' + STRING.ID_REMOVE + id + ']').attr("opacity", 0)
        } else {
            $('circle[id^=' + STRING.ID_CONNECT_TOP_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + STRING.ID_CONNECT_LEFT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + STRING.ID_CONNECT_RIGHT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + STRING.ID_CONNECT_BOTTOM_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('g[id^=' + STRING.ID_REMOVE + ']').each(function () {
                $(this).attr("opacity", 0);
            });
        }
    },
    ...
    moveArrows: function (id) {
        ...
        $.each(_attachArrows, function (i, data) {
            ...
            var _gCard = d3.select("g[id=" + STRING.ID + id + "]");
    ...
    update: function (id) {
        if (id != 0) {
            ...
            var _oCurrent = STRING.get(id);
            var deviceSelect = document.getElementById("stringIds");
            var _deviceId = deviceSelect.value;
            var _deviceName = "";
            if (typeof _oCurrent !== "undefined" && _oCurrent.type == "string") {
    ...
    detail: function (d, data) {
        getOperationInformation(data.customerId, data.deviceId);
    },
