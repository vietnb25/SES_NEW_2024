...
let PANEL2 = {
    ...
    collect: function () {
        ...
        var panel2 = {
            "id": Date.now(),
            "width": PANEL2.DEFAULT_WIDTH,
            "height": PANEL2.DEFAULT_HEIGHT,
            "x": PANEL2.DEFAULT_X,
            "y": PANEL2.DEFAULT_Y,
            "ox": PANEL2.DEFAULT_X,
            "oy": PANEL2.DEFAULT_Y,
    ...
    drawLoad: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            .attr("id", function (d) { return (PANEL2.ID + d.id); })
            ...
            .attr("id", function (d) { return PANEL2.ID_IMG + d.id; })
            .attr("style", "fill:#ddd; opacity: 0.5")
    ...
    draw: function (card) {
        ...
        var _cardTopMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return PANEL2.ID_CONNECT_TOP_MARKER + d.id; })
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", PANEL2.ID_CONNECT_TOP_MARKER)
            .attr("fill", "#276cb8")
            .call(PANEL2.connect);

        var _cardRightMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return PANEL2.ID_CONNECT_RIGHT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", PANEL2.ID_CONNECT_RIGHT_MARKER)
            .attr("fill", "#276cb8")
            .call(PANEL2.connect);

        var _cardBottomMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return PANEL2.ID_CONNECT_BOTTOM_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", function (d) { return d.width / 2; })
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", PANEL2.ID_CONNECT_BOTTOM_MARKER)
            .attr("fill", "#276cb8")
            .call(PANEL2.connect);

        var _cardLeftMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return PANEL2.ID_CONNECT_LEFT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 0)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", PANEL2.ID_CONNECT_LEFT_MARKER)
            .attr("fill", "#276cb8")
            .call(PANEL2.connect);

        var topLeftRezie = _gCard.datum(card).append('path')
            .attr("id", function (d) { return PANEL2.ID_TOP_LEFT_RESIZE + d.id; })
        ...
        var _gX = _gCard.datum(card).append("g")
            .attr("id", function (d) { return (PANEL2.ID_REMOVE + d.id); })
            .attr("transform", "translate(-40 , -50)")
            .attr("opacity", "0")
            .attr("cursor", "pointer")
            .on("click", PANEL2.delete);
    ...
    drag: d3.drag()
        .on("start", function (i, d) {
            ...
            for (var i = 0; i < DATA.aArrows.length; i++) {
                if (DATA.aArrows[i].x >= d.x && DATA.aArrows[i].x <= (d.x + d.width) && 
                DATA.aArrows[i].y >= d.y && DATA.aArrows[i].y <= (d.y + d.height)) {
                    if(DATA.aArrows[i].componentType != "arrow") {
        ...
        .on("drag", function (i, d) {
            var _gCard = d3.select("g[id=" + PANEL2.ID + d.id + "]");
            var _circleTopMaker = d3.select("circle[id=" + PANEL2.ID_CONNECT_TOP_MARKER + d.id + "]");
            var _circleBottomMaker = d3.select("circle[id=" + PANEL2.ID_CONNECT_BOTTOM_MARKER + d.id + "]");
            var _circleLeftMaker = d3.select("circle[id=" + PANEL2.ID_CONNECT_LEFT_MARKER + d.id + "]");
            var _circleRightMaker = d3.select("circle[id=" + PANEL2.ID_CONNECT_RIGHT_MARKER + d.id + "]");
            ...
            if (g_connect.cardId == "") {
                d3.selectAll("circle[id^=" + METER.ID_CIRCLE_CONNECT + "]").each(function (c) {
                    ...
                    if (_isOverlapT || _isOverlapB || _isOverlapL || _isOverlapR) {
                        ...
                        if (_isOverlapT) {
                            g_connect.circleFrom = PANEL2.ID_CONNECT_TOP_MARKER;
                        } else if (_isOverlapB) {
                            g_connect.circleFrom = PANEL2.ID_CONNECT_BOTTOM_MARKER;
                        } else if (_isOverlapL) {
                            g_connect.circleFrom = PANEL2.ID_CONNECT_LEFT_MARKER;
                        } else if (_isOverlapR) {
                            g_connect.circleFrom = PANEL2.ID_CONNECT_RIGHT_MARKER;
                        }
                    };
                });
    ...
    connect: d3.drag()
        .on("start", function (i, d) {
            ...
            var _gArrow = g_svg.datum(PANEL2.directArrow).append('g')
            ...
            var _arrowLine = _gArrow.datum(PANEL2.directArrow).append("line")
            ...
            var _arrowStartPoint = _gArrow.datum(PANEL2.directArrow).append('circle')
            ...
            var _arrowEndPoint = _gArrow.datum(PANEL2.directArrow).append('circle')
        ...
        .on("drag", function (event, d) {
            ...
            var _gArrow = d3.select("g[id=" + ARROW.ID + PANEL2.directArrow.id + "]");
            var _arrowLine = d3.select("line[id=" + ARROW.ID_LINE + PANEL2.directArrow.id + "]");
            var _arrowEndPoint = d3.select("circle[id=" + ARROW.ID_LINE_POINT_END + PANEL2.directArrow.id + "]");
        ...
        .on("end", function (i, d) {
            ...
            if (g_connect.attachFlag && g_connect.id != "") {
                ...
                PANEL2.directArrow.to = g_connect.id;
                PANEL2.directArrow.toMarker = g_connect.attachMarker;
                PANEL2.directArrow.x2 = _x;
                PANEL2.directArrow.y2 = _y;
                DATA.aArrows.push(PANEL2.directArrow);
                ARROW.update(PANEL2.directArrow.id, 1);
            } else {
                d3.select("g[id=" + ARROW.ID + PANEL2.directArrow.id + "]").remove();
            }
    ...
    remove: function (id) {
        ...
        if (!isMarker) {
            d3.select("g[id=" + PANEL2.ID + id + "]").remove();
    ...
    focus: function (id) {
        if (id != null) {
            $('circle[id^=' + PANEL2.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + PANEL2.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + PANEL2.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + PANEL2.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 1);
            $('g[id^=' + PANEL2.ID_ROTATE + id + ']').attr("opacity", 1);
            $('g[id^=' + PANEL2.ID_REMOVE + id + ']').attr("opacity", 1);
            $('path[id^=' + PANEL2.ID_TOP_LEFT_RESIZE + id + ']').attr("opacity", 1);
        }
    },

    outFocus: function (id) {
        if (id != null) {
            $('circle[id^=' + PANEL2.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + PANEL2.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + PANEL2.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + PANEL2.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 0);
            $('g[id^=' + PANEL2.ID_ROTATE + id + ']').attr("opacity", 0);
            $('g[id^=' + PANEL2.ID_REMOVE + id + ']').attr("opacity", 0);
            $('path[id^=' + PANEL2.ID_TOP_LEFT_RESIZE + id + ']').attr("opacity", 0);
        } else {
            $('circle[id^=' + PANEL2.ID_CONNECT_TOP_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + PANEL2.ID_CONNECT_BOTTOM_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + PANEL2.ID_CONNECT_LEFT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + PANEL2.ID_CONNECT_RIGHT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('path[id^=' + PANEL2.ID_TOP_LEFT_RESIZE + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('g[id^=' + PANEL2.ID_REMOVE + ']').each(function () {
                $(this).attr("opacity", 0);
            });
        }
    },
    ...
    moveArrows: function (id) {
        ...
        $.each(_attachArrows, function (i, data) {
            ...
            var _gCard = d3.select("g[id=" + PANEL2.ID + id + "]");
    ...
    update: function (id) {
        ...
        var _oCurrent = PANEL2.get(id);
        if (typeof _oCurrent !== "undefined" && _oCurrent.type == "panel2") {
    ...
    delete: function (e) {
        e.stopPropagation();
        PANEL2.remove(DATA.selectedID);
    },

    resize: d3.drag()
        .on("start", function (i, d) {
            // console.log("I start", i);
            // console.log("Resize start" + "-" + d.x + "-" + d.y + "-" + d.ox + "-" + d.oy + "-" + d.width + "-" + d.height);
            outFocus();
            setAction("");
            DATA.selectedID = d.id;
            DATA.selectedType = 1;
        ...
        .on("drag", function (i, d) {
            ...
            var _circleLeftMaker = d3.select("circle[id=" + PANEL2.ID_CONNECT_LEFT_MARKER + d.id + "]");
            var _circleRightMaker = d3.select("circle[id=" + PANEL2.ID_CONNECT_RIGHT_MARKER + d.id + "]");
            var _img = d3.select("rect[id=" + PANEL2.ID_IMG + d.id + "]");
            let height = d.height;
            let width = d.width;
            ...
            _circleTopMaker.attr("cx", (width / 2))
                .attr("cy", 0);

            _circleBottomMaker.attr("cx", (width / 2))
                .attr("cy", height);

            _circleLeftMaker.attr("cx", 0)
                .attr("cy", (height / 2));

            _circleRightMaker.attr("cx", width)
                .attr("cy", (height / 2));
