...
let KHOANGCHI = {
    ID: "idkhgchi-",
    ID_CIRCLE_CONNECT: "khgic",
    ID_CONNECT_TOP_MARKER: "khgict-",
    ID_CONNECT_LEFT_MARKER: "khgicl-",
    ID_CONNECT_RIGHT_MARKER: "khgicr-",
    ID_CONNECT_BOTTOM_MARKER: "khgicb-",

    ID_PATH_1_KHOANGCHI: "ip1khgchi-",
    ID_PATH_2_KHOANGCHI: "ip2khgchi-",
    ID_REMOVE: "irmkhgchi-",
    ...
    collect: function () {
        var _deviceName = document.getElementById("khoang-chi-device-name").value;
        var _description = document.getElementById("khoang-chi-description").value;
    ...
    create: function () {
        var _oCard = KHOANGCHI.collect();
        DATA.aCards.push(_oCard);
        KHOANGCHI.draw(_oCard);
    },
    ...
    drawLoad: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            .attr("id", function (d) { return (KHOANGCHI.ID + d.id); })
        ...
        var _path1 = _gCard.datum(card).append("path")
            .attr("id", function (d) { return (KHOANGCHI.ID_PATH_1_KHOANGCHI + d.deviceId); })
            .attr("d", CONSTANTS.KHOANGCHI.path1)
    ...
    draw: function (card) {
        ...
        var _cardTopMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return KHOANGCHI.ID_CONNECT_TOP_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", KHOANGCHI.DEFAULT_WIDTH / 2)
            .attr("cy", 2)
            .attr("opacity", 0.5)
            .attr("cursor", "pointer")
            .attr("marker", KHOANGCHI.ID_CONNECT_TOP_MARKER)
            .attr("fill", "#237ba0")
            .call(KHOANGCHI.connect);

        var _cardLeftMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return KHOANGCHI.ID_CONNECT_LEFT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 2)
            .attr("cy", KHOANGCHI.DEFAULT_HEIGHT / 2)
            .attr("opacity", 0.5)
            .attr("cursor", "pointer")
            .attr("marker", KHOANGCHI.ID_CONNECT_LEFT_MARKER)
            .attr("fill", "#237ba0")
            .call(KHOANGCHI.connect);

        var _cardRightMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return KHOANGCHI.ID_CONNECT_RIGHT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", KHOANGCHI.DEFAULT_WIDTH - 2)
            ...
            .attr("opacity", 0.5)
            .attr("cursor", "pointer")
            .attr("marker", KHOANGCHI.ID_CONNECT_RIGHT_MARKER)
            .attr("fill", "#237ba0")
            .call(KHOANGCHI.connect);

        var _cardBottomMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return KHOANGCHI.ID_CONNECT_BOTTOM_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", KHOANGCHI.DEFAULT_WIDTH / 2)
            ...
            .attr("opacity", 0.5)
            .attr("cursor", "pointer")
            .attr("marker", KHOANGCHI.ID_CONNECT_BOTTOM_MARKER)
            .attr("fill", "#237ba0")
            .call(KHOANGCHI.connect);

        var _gX = _gCard.datum(card).append("g")
            .attr("id", function (d) { return (KHOANGCHI.ID_REMOVE + d.id); })
            .attr("transform", "translate(-20,-40)")
            .attr("opacity", "1")
            .attr("cursor", "pointer")
            .on("click", KHOANGCHI.delete);
    ...
    drag: d3.drag()
        .on("start", function (i, d) {
            ...
            var _oCard = KHOANGCHI.get(d.id);
            document.getElementById("khoang-chi-device-name").value = _oCard.deviceName;
            document.getElementById("khoang-chi-description").value = _oCard.description;
        ...
        .on("drag", function (i, d) {
            var _gCard = d3.select("g[id=" + KHOANGCHI.ID + d.id + "]");
            var _circleTopMaker = d3.select("circle[id=" + KHOANGCHI.ID_CONNECT_TOP_MARKER + d.id + "]");
            var _circleLeftMaker = d3.select("circle[id=" + KHOANGCHI.ID_CONNECT_LEFT_MARKER + d.id + "]");
            var _circleRightMaker = d3.select("circle[id=" + KHOANGCHI.ID_CONNECT_RIGHT_MARKER + d.id + "]");
            var _circleBottomMaker = d3.select("circle[id=" + KHOANGCHI.ID_CONNECT_BOTTOM_MARKER + d.id + "]");

            d3.select(this).attr("transform", "translate(" + (d.x) + "," + (d.y) + ")");
            d.x = i.x;
            d.y = i.y;
            KHOANGCHI.moveArrows(d.id);
            ...
            if (g_connect.cardId == "") {
                d3.selectAll("circle[id^=" + METER.ID_CIRCLE_CONNECT + "]").each(function (c) {
                    ...
                    if (_isOverlapT || _isOverlapL || _isOverlapR || _isOverlapB) {
                        ...
                        if (_isOverlapT) {
                            g_connect.circleFrom = KHOANGCHI.ID_CONNECT_TOP_MARKER;
                        } else if (_isOverlapL) {
                            g_connect.circleFrom = KHOANGCHI.ID_CONNECT_LEFT_MARKER;
                        } else if (_isOverlapR) {
                            g_connect.circleFrom = KHOANGCHI.ID_CONNECT_RIGHT_MARKER;
                        } else if (_isOverlapB) {
                            g_connect.circleFrom = KHOANGCHI.ID_CONNECT_BOTTOM_MARKER;
                        };
                    };
                });
    ...
    connect: d3.drag()
        .on("start", function (i, d) {
            ...
            var _gArrow = g_svg.datum(KHOANGCHI.directArrow).append('g')
            ...
            var _arrowLine = _gArrow.datum(KHOANGCHI.directArrow).append("line")
            ...
            var _arrowStartPoint = _gArrow.datum(KHOANGCHI.directArrow).append('circle')
            ...
            var _arrowEndPoint = _gArrow.datum(KHOANGCHI.directArrow).append('circle')
        ...
        .on("drag", function (event, d) {
            ...
            var _gArrow = d3.select("g[id=" + ARROW.ID + KHOANGCHI.directArrow.id + "]");
            var _arrowLine = d3.select("line[id=" + ARROW.ID_LINE + KHOANGCHI.directArrow.id + "]");
            var _arrowEndPoint = d3.select("circle[id=" + ARROW.ID_LINE_POINT_END + KHOANGCHI.directArrow.id + "]");
        ...
        .on("end", function (i, d) {
            ...
            if (g_connect.attachFlag && g_connect.id != "") {
                ...
                KHOANGCHI.directArrow.to = g_connect.id;
                KHOANGCHI.directArrow.toMarker = g_connect.attachMarker;
                KHOANGCHI.directArrow.x2 = _x;
                KHOANGCHI.directArrow.y2 = _y;
                DATA.aArrows.push(KHOANGCHI.directArrow);
                ARROW.update(KHOANGCHI.directArrow.id, 1);
            } else {
                d3.select("g[id=" + ARROW.ID + KHOANGCHI.directArrow.id + "]").remove();
            }
    ...
    remove: function (id) {
        ...
        if (!isMarker) {
            d3.select("g[id=" + KHOANGCHI.ID + id + "]").remove();
    ...
    focus: function (id) {
        if (id != null) {
            $('circle[id^=' + KHOANGCHI.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + KHOANGCHI.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + KHOANGCHI.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + KHOANGCHI.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 1);
            $('g[id^=' + KHOANGCHI.ID_REMOVE + id + ']').attr("opacity", 1);
        }
    },

    outFocus: function (id) {
        if (id != null) {
            $('circle[id^=' + KHOANGCHI.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + KHOANGCHI.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + KHOANGCHI.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + KHOANGCHI.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 0);
            $('g[id^=' + KHOANGCHI.ID_REMOVE + id + ']').attr("opacity", 0)
        } else {
            $('circle[id^=' + KHOANGCHI.ID_CONNECT_TOP_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + KHOANGCHI.ID_CONNECT_LEFT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + KHOANGCHI.ID_CONNECT_RIGHT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + KHOANGCHI.ID_CONNECT_BOTTOM_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('g[id^=' + KHOANGCHI.ID_REMOVE + ']').each(function () {
                $(this).attr("opacity", 0);
            });
        }
    },
    ...
    moveArrows: function (id) {
        ...
        $.each(_attachArrows, function (i, data) {
            ...
            var _gCard = d3.select("g[id=" + KHOANGCHI.ID + id + "]");
    ...
    update: function (id) {
        var _oCurrent = KHOANGCHI.get(id);

        if (typeof _oCurrent !== "undefined" && _oCurrent.type == "khoangchi") {
            var _deviceName = document.getElementById("khoang-chi-device-name").value;
            var _description = document.getElementById("khoang-chi-description").value;
            var _oCard = {
                "id": _oCurrent.id,
