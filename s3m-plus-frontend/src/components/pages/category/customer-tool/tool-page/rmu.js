...
let RMU = {
    ID: "grmu-",
    ID_CIRCLE_CONNECT: "rmuc",
    ID_CONNECT_LEFT_MARKER: "rmucl-",
    ID_CONNECT_MIDDLE_MARKER: "rmucm-",
    ID_CONNECT_RIGHT_MARKER: "rmucr-",
    ID_REMOVE: "rmurm",
    directArrow: {},

    collect: function () {
        var _deviceName = document.getElementById("mba-device-name").value;
        var _description = document.getElementById("rmu-description").value;
    ...
    drawLoad: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            .attr("id", function (d) { return (RMU.ID + d.id); })
    ...
    draw: function (card) {
        ...
        var _cardLeftMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return RMU.ID_CONNECT_LEFT_MARKER + d.id; })
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", RMU.ID_CONNECT_LEFT_MARKER)
            .attr("fill", "#237ba0")
            .call(RMU.connect);

        var _cardMiddleMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return RMU.ID_CONNECT_MIDDLE_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 123)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", RMU.ID_CONNECT_MIDDLE_MARKER)
            .attr("fill", "#237ba0")
            .call(RMU.connect);

        var _cardRightMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return RMU.ID_CONNECT_RIGHT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 185)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", RMU.ID_CONNECT_RIGHT_MARKER)
            .attr("fill", "#237ba0")
            .call(RMU.connect);

        var _gX = _gCard.datum(card).append("g")
            .attr("id", function (d) { return (RMU.ID_REMOVE + d.id); })
    ...
    drag: d3.drag()
        .on("start", function (i, d) {
            ...
            var _oCard = RMU.get(d.id);
            document.getElementById("rmu-device-name").value = _oCard.deviceName;
            document.getElementById("rmu-description").value = _oCard.description;
        ...
        .on("drag", function (i, d) {
            var _gCard = d3.select("g[id=" + RMU.ID + d.id + "]");
            var _circleLeftMaker = d3.select("circle[id=" + RMU.ID_CONNECT_LEFT_MARKER + d.id + "]");
            var _circleMiddleMaker = d3.select("circle[id=" + RMU.ID_CONNECT_MIDDLE_MARKER + d.id + "]");
            var _circleRightMaker = d3.select("circle[id=" + RMU.ID_CONNECT_RIGHT_MARKER + d.id + "]");
            ...
            if (g_connect.cardId == "") {
                d3.selectAll("circle[id^=" + METER.ID_CIRCLE_CONNECT + "]").each(function (c) {
                    ...
                    var _isOverlapM = COMMON.intersectRect(_circleMiddleMaker.node(), this);
                    var _isOverlapR = COMMON.intersectRect(_circleRightMaker.node(), this);
                    if (_isOverlapL || _isOverlapM || _isOverlapR) {
                        g_connect.id = this.id;
                        g_connect.cardId = this.id.split("-").pop();
                        g_connect.type = "meter";
                        if (_isOverlapL) {
                            g_connect.circleFrom = RMU.ID_CONNECT_LEFT_MARKER;
                        } else if (_isOverlapM) {
                            g_connect.circleFrom = RMU.ID_CONNECT_MIDDLE_MARKER;
                        } else if (_isOverlapR) {
                            g_connect.circleFrom = RMU.ID_CONNECT_RIGHT_MARKER;
                        };
                    };
                });
            ...
            if (g_connect.cardId != "") {
                ...
                var _isOverlapM = COMMON.intersectRect(_circleMiddleMaker.node(), _circleConnect.node());
    ...
    connect: d3.drag()
        .on("start", function (i, d) {
            ...
            var _gArrow = g_svg.datum(RMU.directArrow).append('g')
            ...
            var _arrowLine = _gArrow.datum(RMU.directArrow).append("line")
            ...
            var _arrowStartPoint = _gArrow.datum(RMU.directArrow).append('circle')
            ...
            var _arrowEndPoint = _gArrow.datum(RMU.directArrow).append('circle')
        ...
        .on("drag", function (event, d) {
            ...
            var _gArrow = d3.select("g[id=" + ARROW.ID + RMU.directArrow.id + "]");
            var _arrowLine = d3.select("line[id=" + ARROW.ID_LINE + RMU.directArrow.id + "]");
            var _arrowEndPoint = d3.select("circle[id=" + ARROW.ID_LINE_POINT_END + RMU.directArrow.id + "]");
        ...
        .on("end", function (i, d) {
            ...
            if (g_connect.attachFlag && g_connect.id != "") {
                ...
                RMU.directArrow.to = g_connect.id;
                RMU.directArrow.toMarker = g_connect.attachMarker;
                RMU.directArrow.x2 = _x;
                RMU.directArrow.y2 = _y;
                DATA.aArrows.push(RMU.directArrow);
                ARROW.update(RMU.directArrow.id, 1);
            } else {
                d3.select("g[id=" + ARROW.ID + RMU.directArrow.id + "]").remove();
            }
    ...
    remove: function (id) {
        ...
        if (!isMarker) {
            d3.select("g[id=" + RMU.ID + id + "]").remove();
    ...
    focus: function (id) {
        if (id != null) {
            $('circle[id^=' + RMU.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + RMU.ID_CONNECT_MIDDLE_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + RMU.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 1);
            $('g[id^=' + RMU.ID_REMOVE + id + ']').attr("opacity", 1)
        }
    },

    outFocus: function (id) {
        if (id != null) {
            $('circle[id^=' + RMU.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + RMU.ID_CONNECT_MIDDLE_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + RMU.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 0);
            $('g[id^=' + RMU.ID_REMOVE + id + ']').attr("opacity", 0);
        } else {
            $('circle[id^=' + RMU.ID_CONNECT_LEFT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + RMU.ID_CONNECT_MIDDLE_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + RMU.ID_CONNECT_RIGHT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('g[id^=' + RMU.ID_REMOVE + ']').each(function () {
                $(this).attr("opacity", 0);
            });
        }
    },
    ...
    moveArrows: function (id) {
        ...
        $.each(_attachArrows, function (i, data) {
            ...
            var _gCard = d3.select("g[id=" + RMU.ID + id + "]");
    ...
    update: function (id) {
        ...
        var _oCurrent = RMU.get(id);

        if (typeof _oCurrent !== "undefined" && _oCurrent.type == "rmu") {
            var _deviceName = document.getElementById("rmu-device-name").value;
            var _description = document.getElementById("rmu-description").value;
            var _oCard = {
                "id": _oCurrent.id,
                "deviceName": _deviceName,
