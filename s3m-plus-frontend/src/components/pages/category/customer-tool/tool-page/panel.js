...
let PANEL = {
    ID: "idpanel-",
    ID_CIRCLE_CONNECT: "panelc",
    ID_CONNECT_TOP_MARKER: "panelct-",
    ID_CONNECT_LEFT_MARKER: "panelcl-",
    ID_CONNECT_RIGHT_MARKER: "panelcr-",
    ID_CONNECT_BOTTOM_MARKER: "panelcb-",
    ID_STATUS_PANEL: "ispanel-",
    ID_PATH_PANEL: "ippanel-",
    ID_REMOVE: "irmpanel-",
    ID_DETAIL: "idtpanel-",
    ID_FOCUS: "ifpanel-",
    ...
    ID_PANEL_VALUE_U: "ivupanel-",
    ID_PANEL_VALUE_I: "ivipanel-",
    ID_PANEL_VALUE_T: "ivppanel-",

    directArrow: {},

    collect: function () {
        var _tagName = document.getElementById("panel-tag-name").value;
        var _description = document.getElementById("panel-description").value;
        var _deviceCaculator = Number($("input[name=panel_caculator]").filter(":checked").val());
    ...
    drawLoad: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            .attr("id", function (d) { return (PANEL.ID + d.id); })
        ...
        var _path1 = _gCard.datum(card).append("path")
            .attr("id", function (d) { return (PANEL.ID_PATH_PANEL + d.deviceId); })
        ...
        var _statusPath = _gCard.datum(card).append("path")
            .attr("id", function (d) { return (PANEL.ID_STATUS_PANEL + d.deviceId); })
            .attr("d", "")
        ...
        var _valueU = _gCard.datum(card).append("text")
            .attr("id", function (d) { return (PANEL.ID_PANEL_VALUE_U + d.deviceId); })
            .attr("x", "99")
            .attr("y", "30")
            .attr("fill", "black")
            .attr("font-size", "12")
        ...
        var _valueI = _gCard.datum(card).append("text")
            .attr("id", function (d) { return (PANEL.ID_PANEL_VALUE_I + d.deviceId); })
            .attr("x", "99")
            .attr("y", "48")
            .attr("fill", "black")
            .attr("font-size", "12")
        ...
        var _valueT = _gCard.datum(card).append("text")
            .attr("id", function (d) { return (PANEL.ID_PANEL_VALUE_T + d.deviceId); })
            .attr("x", "99")
            .attr("y", "67")
            .attr("fill", "black")
            .attr("font-size", "12")
        ...
        var _rect = _gCard.datum(card).append("rect")
            .attr("id", function (d) { return (PANEL.ID_FOCUS + d.id); })
            .attr("x", "-20")
            .attr("y", "-20")
            .attr("width", "300")
            .attr("height", "160")
    ...
    draw: function (card) {
        ...
        var _cardTopMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return PANEL.ID_CONNECT_TOP_MARKER + d.id; })
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", PANEL.ID_CONNECT_TOP_MARKER)
            .attr("fill", "#237ba0")
            .call(PANEL.connect);

        var _cardLeftMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return PANEL.ID_CONNECT_LEFT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 9)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", PANEL.ID_CONNECT_LEFT_MARKER)
            .attr("fill", "#237ba0")
            .call(PANEL.connect);

        var _cardRightMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return PANEL.ID_CONNECT_RIGHT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 78)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", PANEL.ID_CONNECT_RIGHT_MARKER)
            .attr("fill", "#237ba0")
            .call(PANEL.connect);

        var _cardBottomMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return PANEL.ID_CONNECT_BOTTOM_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 43.5)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", PANEL.ID_CONNECT_BOTTOM_MARKER)
            .attr("fill", "#237ba0")
            .call(PANEL.connect);

        var _gX = _gCard.datum(card).append("g")
            .attr("id", function (d) { return (PANEL.ID_REMOVE + d.id); })
            .attr("transform", "translate(-20,-40)")
            .attr("opacity", "0.5")
            .attr("cursor", "pointer")
            .on("click", PANEL.delete);
    ...
    drag: d3.drag()
        .on("start", function (i, d) {
            ...
            var selectElement = document.querySelectorAll('[id=panelIds]');
            ...
            if (typeof deviceIdOption === "undefined" && deviceIdOption != "") {
                $("#panelIds").append(new Option(_oCard.deviceName, _oCard.deviceId));
            }
            document.getElementById("panelIds").value = _oCard.deviceId;

            if (Number(_oCard.caculator) == 1) {
                document.getElementById("panel-caculator-1").checked = true;
            } else {
                document.getElementById("panel-caculator-0").checked = true;
            };
        ...
        .on("drag", function (i, d) {
            var _gCard = d3.select("g[id=" + PANEL.ID + d.id + "]");
            var _circleTopMaker = d3.select("circle[id=" + PANEL.ID_CONNECT_TOP_MARKER + d.id + "]");
            var _circleLeftMaker = d3.select("circle[id=" + PANEL.ID_CONNECT_LEFT_MARKER + d.id + "]");
            var _circleRightMaker = d3.select("circle[id=" + PANEL.ID_CONNECT_LEFT_MARKER + d.id + "]");
            var _circleBottomMaker = d3.select("circle[id=" + PANEL.ID_CONNECT_BOTTOM_MARKER + d.id + "]");

            d3.select(this).attr("transform", "translate(" + (d.x) + "," + (d.y) + ")");
            d.x = i.x;
            d.y = i.y;
            PANEL.moveArrows(d.id);
            ...
            if (g_connect.cardId == "") {
                d3.selectAll("circle[id^=" + METER.ID_CIRCLE_CONNECT + "]").each(function (c) {
                    ...
                    if (_isOverlapT || _isOverlapL || _isOverlapR || _isOverlapB) {
                        ...
                        if (_isOverlapT) {
                            g_connect.circleFrom = PANEL.ID_CONNECT_TOP_MARKER;
                        } else if (_isOverlapL) {
                            g_connect.circleFrom = PANEL.ID_CONNECT_LEFT_MARKER;
                        } else if (_isOverlapR) {
                            g_connect.circleFrom = PANEL.ID_CONNECT_RIGHT_MARKER;
                        } else if (_isOverlapB) {
                            g_connect.circleFrom = PANEL.ID_CONNECT_BOTTOM_MARKER;
                        };
                    };
                });
    ...
    connect: d3.drag()
        .on("start", function (i, d) {
            ...
            var _gArrow = g_svg.datum(PANEL.directArrow).append('g')
            ...
            var _arrowLine = _gArrow.datum(PANEL.directArrow).append("line")
            ...
            var _arrowStartPoint = _gArrow.datum(PANEL.directArrow).append('circle')
            ...
            var _arrowEndPoint = _gArrow.datum(PANEL.directArrow).append('circle')
        ...
        .on("drag", function (event, d) {
            ...
            var _gArrow = d3.select("g[id=" + ARROW.ID + PANEL.directArrow.id + "]");
            var _arrowLine = d3.select("line[id=" + ARROW.ID_LINE + PANEL.directArrow.id + "]");
            var _arrowEndPoint = d3.select("circle[id=" + ARROW.ID_LINE_POINT_END + PANEL.directArrow.id + "]");
        ...
        .on("end", function (i, d) {
            ...
            if (g_connect.attachFlag && g_connect.id != "") {
                ...
                PANEL.directArrow.to = g_connect.id;
                PANEL.directArrow.toMarker = g_connect.attachMarker;
                PANEL.directArrow.x2 = _x;
                PANEL.directArrow.y2 = _y;
                DATA.aArrows.push(PANEL.directArrow);
                ARROW.update(PANEL.directArrow.id, 1);
            } else {
                d3.select("g[id=" + ARROW.ID + PANEL.directArrow.id + "]").remove();
            }
    ...
    remove: function (id) {
        ...
        if (!isMarker) {
            d3.select("g[id=" + PANEL.ID + id + "]").remove();
    ...
    focus: function (id) {
        if (id != null) {
            $('circle[id^=' + PANEL.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + PANEL.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + PANEL.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + PANEL.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 1);
            $('g[id^=' + PANEL.ID_REMOVE + id + ']').attr("opacity", 1);
        }
    },

    outFocus: function (id) {
        if (id != null) {
            $('circle[id^=' + PANEL.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + PANEL.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + PANEL.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + PANEL.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 0);
            $('g[id^=' + PANEL.ID_REMOVE + id + ']').attr("opacity", 0)
        } else {
            $('circle[id^=' + PANEL.ID_CONNECT_TOP_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + PANEL.ID_CONNECT_LEFT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + PANEL.ID_CONNECT_RIGHT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + PANEL.ID_CONNECT_BOTTOM_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('g[id^=' + PANEL.ID_REMOVE + ']').each(function () {
                $(this).attr("opacity", 0);
            });
        }
    },
    ...
    moveArrows: function (id) {
        ...
        $.each(_attachArrows, function (i, data) {
            ...
            var _gCard = d3.select("g[id=" + PANEL.ID + id + "]");
    ...
    update: function (id) {
        if (id != 0) {
            ...
            var _oCurrent = PANEL.get(id);
            var deviceSelect = document.getElementById("panelIds");
            var _deviceId = deviceSelect.value;
            var _deviceName = "";
            if (typeof _oCurrent !== "undefined" && _oCurrent.type == "panel") {
