...
let STRINGSYMBOL = {
    ID: "idstrsb-",
    ID_CIRCLE_CONNECT: "strsblc",
    ID_CONNECT_TOP_MARKER: "strsblct-",
    ID_CONNECT_LEFT_MARKER: "strsblcl-",
    ID_CONNECT_RIGHT_MARKER: "strsblcr-",
    ID_CONNECT_BOTTOM_MARKER: "strsblcb-",
    ID_REMOVE: "strsbrm-",
    directArrow: {},

    collect: function () {
        var _deviceName = document.getElementById("string-symbol-device-name").value;
        var _description = document.getElementById("string-symbol-description").value;
    ...
    create: function () {
        var _oCard = STRINGSYMBOL.collect();
        DATA.aCards.push(_oCard);
        STRINGSYMBOL.draw(_oCard);
    },
    ...
    drawLoad: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            .attr("id", function (d) { return (STRINGSYMBOL.ID + d.id); })
            ...
            .on("click", STRINGSYMBOL.select)
            .call(STRINGSYMBOL.drag);
        ...
        var _path1 = _gCard.datum(card).append("path")
            .attr("d", CONSTANTS.STRING_SYMBOL.path1)
    ...
    draw: function (card) {
        ...
        var _cardTopMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return STRINGSYMBOL.ID_CONNECT_TOP_MARKER + d.id; })
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", STRINGSYMBOL.ID_CONNECT_TOP_MARKER)
            .attr("fill", "#237ba0")
            .call(STRINGSYMBOL.connect);

        var _cardLeftMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return STRINGSYMBOL.ID_CONNECT_LEFT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 1.5)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", STRINGSYMBOL.ID_CONNECT_LEFT_MARKER)
            .attr("fill", "#237ba0")
            .call(STRINGSYMBOL.connect);

        var _cardRightMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return STRINGSYMBOL.ID_CONNECT_RIGHT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 34)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", STRINGSYMBOL.ID_CONNECT_RIGHT_MARKER)
            .attr("fill", "#237ba0")
            .call(STRINGSYMBOL.connect);

        var _cardBottomMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return STRINGSYMBOL.ID_CONNECT_BOTTOM_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 18)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", STRINGSYMBOL.ID_CONNECT_BOTTOM_MARKER)
            .attr("fill", "#237ba0")
            .call(STRINGSYMBOL.connect);

        var _gX = _gCard.datum(card).append("g")
            .attr("id", function (d) { return (STRINGSYMBOL.ID_REMOVE + d.id); })
            .attr("transform", "translate(-20,-30)")
            .attr("opacity", "0")
            .attr("cursor", "pointer")
            .on("click", STRINGSYMBOL.delete);
    ...
    drag: d3.drag()
        .on("start", function (i, d) {
            ...
            var _oCard = STRINGSYMBOL.get(d.id);
            document.getElementById("string-symbol-device-name").value = _oCard.deviceName;
            document.getElementById("string-symbol-description").value = _oCard.description;
        ...
        .on("drag", function (i, d) {
            var _gCard = d3.select("g[id=" + STRINGSYMBOL.ID + d.id + "]");
            var _circleTopMaker = d3.select("circle[id=" + STRINGSYMBOL.ID_CONNECT_TOP_MARKER + d.id + "]");
            var _circleLeftMaker = d3.select("circle[id=" + STRINGSYMBOL.ID_CONNECT_LEFT_MARKER + d.id + "]");
            var _circleRightMaker = d3.select("circle[id=" + STRINGSYMBOL.ID_CONNECT_RIGHT_MARKER + d.id + "]");
            var _circleBottomMaker = d3.select("circle[id=" + STRINGSYMBOL.ID_CONNECT_BOTTOM_MARKER + d.id + "]");

            d3.select(this).attr("transform", "translate(" + (d.x) + "," + (d.y) + ")");
            d.x = i.x;
            d.y = i.y;
            STRINGSYMBOL.moveArrows(d.id);
            ...
            if (g_connect.cardId == "") {
                d3.selectAll("circle[id^=" + METER.ID_CIRCLE_CONNECT + "]").each(function (c) {
                    ...
                    if (_isOverlapT || _isOverlapL || _isOverlapR || _isOverlapB) {
                        ...
                        if (_isOverlapT) {
                            g_connect.circleFrom = STRINGSYMBOL.ID_CONNECT_TOP_MARKER;
                        } else if (_isOverlapL) {
                            g_connect.circleFrom = STRINGSYMBOL.ID_CONNECT_LEFT_MARKER;
                        } else if (_isOverlapR) {
                            g_connect.circleFrom = STRINGSYMBOL.ID_CONNECT_RIGHT_MARKER;
                        } else if (_isOverlapB) {
                            g_connect.circleFrom = STRINGSYMBOL.ID_CONNECT_BOTTOM_MARKER;
                        };
                    };
                });
    ...
    connect: d3.drag()
        .on("start", function (i, d) {
            ...
            var _gArrow = g_svg.datum(STRINGSYMBOL.directArrow).append('g')
            ...
            var _arrowLine = _gArrow.datum(STRINGSYMBOL.directArrow).append("line")
            ...
            var _arrowStartPoint = _gArrow.datum(STRINGSYMBOL.directArrow).append('circle')
            ...
            var _arrowEndPoint = _gArrow.datum(STRINGSYMBOL.directArrow).append('circle')
        ...
        .on("drag", function (event, d) {
            ...
            var _gArrow = d3.select("g[id=" + ARROW.ID + STRINGSYMBOL.directArrow.id + "]");
            var _arrowLine = d3.select("line[id=" + ARROW.ID_LINE + STRINGSYMBOL.directArrow.id + "]");
            var _arrowEndPoint = d3.select("circle[id=" + ARROW.ID_LINE_POINT_END + STRINGSYMBOL.directArrow.id + "]");
        ...
        .on("end", function (i, d) {
            ...
            if (g_connect.attachFlag && g_connect.id != "") {
                ...
                STRINGSYMBOL.directArrow.to = g_connect.id;
                STRINGSYMBOL.directArrow.toMarker = g_connect.attachMarker;
                STRINGSYMBOL.directArrow.x2 = _x;
                STRINGSYMBOL.directArrow.y2 = _y;
                DATA.aArrows.push(STRINGSYMBOL.directArrow);
                ARROW.update(STRINGSYMBOL.directArrow.id, 1);
            } else {
                d3.select("g[id=" + ARROW.ID + STRINGSYMBOL.directArrow.id + "]").remove();
            }
    ...
    remove: function (id) {
        ...
        if (!isMarker) {
            d3.select("g[id=" + STRINGSYMBOL.ID + id + "]").remove();
    ...
    focus: function (id) {
        if (id != null) {
            $('circle[id^=' + STRINGSYMBOL.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + STRINGSYMBOL.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + STRINGSYMBOL.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + STRINGSYMBOL.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 1);
            $('g[id^=' + STRINGSYMBOL.ID_REMOVE + id + ']').attr("opacity", 1);
        }
    },

    outFocus: function (id) {
        if (id != null) {
            $('circle[id^=' + STRINGSYMBOL.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + STRINGSYMBOL.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + STRINGSYMBOL.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + STRINGSYMBOL.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 0);
            $('g[id^=' + STRINGSYMBOL.ID_REMOVE + id + ']').attr("opacity", 0);
        } else {
            $('circle[id^=' + STRINGSYMBOL.ID_CONNECT_TOP_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + STRINGSYMBOL.ID_CONNECT_LEFT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + STRINGSYMBOL.ID_CONNECT_RIGHT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + STRINGSYMBOL.ID_CONNECT_BOTTOM_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('g[id^=' + STRINGSYMBOL.ID_REMOVE + ']').each(function () {
                $(this).attr("opacity", 0);
            });
        }
    },
    ...
    moveArrows: function (id) {
        ...
        $.each(_attachArrows, function (i, data) {
            ...
            var _gCard = d3.select("g[id=" + STRINGSYMBOL.ID + id + "]");
    ...
    update: function (id) {
        ...
        var _oCurrent = STRINGSYMBOL.get(id);

        if (typeof _oCurrent !== "undefined" && _oCurrent.type == "stringsymbol") {
            var _deviceName = document.getElementById("string-symbol-device-name").value;
            var _description = document.getElementById("string-symbol-description").value;
            var _oCard = {
                "id": _oCurrent.id,
