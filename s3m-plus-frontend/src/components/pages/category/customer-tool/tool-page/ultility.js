...
let ULTILITY = {
    ID: "uli-",
    ID_CIRCLE_CONNECT: "ulc",
    ID_CONNECT_TOP_MARKER: "ulct-",
    ID_CONNECT_RIGHT_MARKER: "ulcr-",
    ID_CONNECT_BOTTOM_MARKER: "ulcb-",
    ID_CONNECT_LEFT_MARKER: "ulcl-",
    ID_REMOVE: "ulrm",
    ...
    collect: function () {
        var _tagName = document.getElementById("ultility-tag-name").value;
        var _description = document.getElementById("ultility-description").value;

        var _ultility = {
            "id": Date.now(),
            "tagName": _tagName,
            "description": _description,
            "width": ULTILITY.DEFAULT_WIDTH,
            "height": ULTILITY.DEFAULT_HEIGHT,
            "x": ULTILITY.DEFAULT_X,
            "y": ULTILITY.DEFAULT_Y,
            "type": "ultility",
        };
        return _ultility;
    },
    ...
    create: function () {
        var _oCard = ULTILITY.collect();
        DATA.aCards.push(_oCard);
        ULTILITY.draw(_oCard);
    },
    ...
    drawLoad: function (card) {
        ...
        var _gCard = g_svg.datum(card).append('g')
            .attr("id", function (d) { return (ULTILITY.ID + d.id); })
        ...
        var _rect = _gCard.datum(card).append("rect")
            .attr("x", "0")
            .attr("y", "0")
            .attr("width", function (d) { return ULTILITY.DEFAULT_WIDTH })
            .attr("height", function (d) { return ULTILITY.DEFAULT_HEIGHT })
            .attr("fill", "transparent");

        var _path1 = _gCard.datum(card).append("path")
            .attr("d", CONSTANTS.ULTILITY.path1)
    ...
    draw: function (card) {
        ...
        var _cardTopMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return ULTILITY.ID_CONNECT_TOP_MARKER + d.id; })
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", ULTILITY.ID_CONNECT_TOP_MARKER)
            .attr("fill", "#276cb8")
            .call(ULTILITY.connect);

        var _cardRightMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return ULTILITY.ID_CONNECT_RIGHT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", function (d) { return d.width - 2; })
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", ULTILITY.ID_CONNECT_RIGHT_MARKER)
            .attr("fill", "#276cb8")
            .call(ULTILITY.connect);

        var _cardBottomMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return ULTILITY.ID_CONNECT_BOTTOM_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", function (d) { return d.width / 2; })
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", ULTILITY.ID_CONNECT_BOTTOM_MARKER)
            .attr("fill", "#276cb8")
            .call(ULTILITY.connect);

        var _cardLeftMaker = _gCard.datum(card).append('circle')
            .attr("id", function (d) { return ULTILITY.ID_CONNECT_LEFT_MARKER + d.id; })
            .attr("class", "circle-connect")
            .attr("r", "7")
            .attr("transform", "translate(0,0)")
            .attr("cx", 2)
            ...
            .attr("opacity", 0)
            .attr("cursor", "pointer")
            .attr("marker", ULTILITY.ID_CONNECT_LEFT_MARKER)
            .attr("fill", "#276cb8")
            .call(ULTILITY.connect);

        var _gX = _gCard.datum(card).append("g")
            .attr("id", function (d) { return (ULTILITY.ID_REMOVE + d.id); })
            .attr("transform", "translate(-20,-40)")
            .attr("opacity", "0")
            .attr("cursor", "pointer")
            .on("click", ULTILITY.delete);
    ...
    drag: d3.drag()
        .on("start", function (i, d) {
            ...
            var _oCard = ULTILITY.get(d.id);
            document.getElementById("ultility-tag-name").value = _oCard.tagName;
            document.getElementById("ultility-description").value = _oCard.description;
        ...
        .on("drag", function (i, d) {
            var _gCard = d3.select("g[id=" + ULTILITY.ID + d.id + "]");
            var _circleTopMaker = d3.select("circle[id=" + ULTILITY.ID_CONNECT_TOP_MARKER + d.id + "]");
            var _circleBottomMaker = d3.select("circle[id=" + ULTILITY.ID_CONNECT_BOTTOM_MARKER + d.id + "]");
            var _circleLeftMaker = d3.select("circle[id=" + ULTILITY.ID_CONNECT_LEFT_MARKER + d.id + "]");
            var _circleRightMaker = d3.select("circle[id=" + ULTILITY.ID_CONNECT_RIGHT_MARKER + d.id + "]");

            d3.select(this).attr("transform", "translate(" + (d.x) + "," + (d.y) + ")");
            d.x = i.x;
            d.y = i.y;
            ULTILITY.moveArrows(d.id);
            ...
            if (g_connect.cardId == "") {
                d3.selectAll("circle[id^=" + METER.ID_CIRCLE_CONNECT + "]").each(function (c) {
                    ...
                    if (_isOverlapT || _isOverlapB || _isOverlapL || _isOverlapR) {
                        ...
                        if (_isOverlapT) {
                            g_connect.circleFrom = ULTILITY.ID_CONNECT_TOP_MARKER;
                        } else if (_isOverlapB) {
                            g_connect.circleFrom = ULTILITY.ID_CONNECT_BOTTOM_MARKER;
                        } else if (_isOverlapL) {
                            g_connect.circleFrom = ULTILITY.ID_CONNECT_LEFT_MARKER;
                        } else if (_isOverlapR) {
                            g_connect.circleFrom = ULTILITY.ID_CONNECT_RIGHT_MARKER;
                        }
                    };
                });
    ...
    connect: d3.drag()
        .on("start", function (i, d) {
            ...
            var _gArrow = g_svg.datum(ULTILITY.directArrow).append('g')
            ...
            var _arrowLine = _gArrow.datum(ULTILITY.directArrow).append("line")
            ...
            var _arrowStartPoint = _gArrow.datum(ULTILITY.directArrow).append('circle')
            ...
            var _arrowEndPoint = _gArrow.datum(ULTILITY.directArrow).append('circle')
        ...
        .on("drag", function (event, d) {
            ...
            var _gArrow = d3.select("g[id=" + ARROW.ID + ULTILITY.directArrow.id + "]");
            var _arrowLine = d3.select("line[id=" + ARROW.ID_LINE + ULTILITY.directArrow.id + "]");
            var _arrowEndPoint = d3.select("circle[id=" + ARROW.ID_LINE_POINT_END + ULTILITY.directArrow.id + "]");
        ...
        .on("end", function (i, d) {
            ...
            if (g_connect.attachFlag && g_connect.id != "") {
                ...
                ULTILITY.directArrow.to = g_connect.id;
                ULTILITY.directArrow.toMarker = g_connect.attachMarker;
                ULTILITY.directArrow.x2 = _x;
                ULTILITY.directArrow.y2 = _y;
                DATA.aArrows.push(ULTILITY.directArrow);
                ARROW.update(ULTILITY.directArrow.id, 1);
            } else {
                d3.select("g[id=" + ARROW.ID + ULTILITY.directArrow.id + "]").remove();
            }
    ...
    remove: function (id) {
        ...
        if (!isMarker) {
            d3.select("g[id=" + ULTILITY.ID + id + "]").remove();
    ...
    focus: function (id) {
        if (id != null) {
            $('circle[id^=' + ULTILITY.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + ULTILITY.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + ULTILITY.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 1);
            $('circle[id^=' + ULTILITY.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 1);
            $('g[id^=' + ULTILITY.ID_REMOVE + id + ']').attr("opacity", 1);
        }
    },

    outFocus: function (id) {
        if (id != null) {
            $('circle[id^=' + ULTILITY.ID_CONNECT_TOP_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + ULTILITY.ID_CONNECT_BOTTOM_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + ULTILITY.ID_CONNECT_LEFT_MARKER + id + ']').attr("opacity", 0);
            $('circle[id^=' + ULTILITY.ID_CONNECT_RIGHT_MARKER + id + ']').attr("opacity", 0);
            $('g[id^=' + ULTILITY.ID_REMOVE + id + ']').attr("opacity", 0);
        } else {
            $('circle[id^=' + ULTILITY.ID_CONNECT_TOP_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + ULTILITY.ID_CONNECT_BOTTOM_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + ULTILITY.ID_CONNECT_LEFT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('circle[id^=' + ULTILITY.ID_CONNECT_RIGHT_MARKER + ']').each(function () {
                $(this).attr("opacity", 0);
            });
            $('g[id^=' + ULTILITY.ID_REMOVE + ']').each(function () {
                $(this).attr("opacity", 0);
            });
        }
    },
    ...
    moveArrows: function (id) {
        ...
        $.each(_attachArrows, function (i, data) {
            ...
            var _gCard = d3.select("g[id=" + ULTILITY.ID + id + "]");
    ...
    update: function (id) {
        var _oCurrent = ULTILITY.get(id);

        if (typeof _oCurrent !== "undefined" && _oCurrent.type == "ultility") {
            var _tagName = document.getElementById("ultility-tag-name").value;
            var _description = document.getElementById("ultility-description").value;
            var _oCard = {};
