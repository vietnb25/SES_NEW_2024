...
const CambienAny = (props) => {
    const initialValues = {
        ...
        fuelFormId: props.device.fuelFormId,
    ...
    const [oldMaterialTypes, setOldMaterialTypes] = useState([]);
    ...
    const formik = useFormik({
        ...
        initialValues,
        onSubmit: async (data) => {
            data.fuelFormId = Number(document.getElementById("fuelFormId").value);
            data.fuelTypeId = Number(document.getElementById("fuelTypeId").value);
    ...
    const getListMaterialType = async () => {
        ...
        if (res.status == 200) {
            setMaterialTypes(res.data.filter(item => item.materialForm == props.device.fuelFormId))
            setOldMaterialTypes(res.data);
        }
    }
    const changefuelFormId = (id) => {
        let arr = oldMaterialTypes.filter(item => item.materialForm == id);
                ...
                <table className={`table table-input `}>
                    <tbody>
                        ...
                        <tr className="object-type">
                            ...
                            <td>
                                <select className="custom-select block object-type-id" id="object-type-id" onChange={formik.handleChange} value={formik.values.objectId}>
                                    {
                        ...
                        <tr>
                            <th width="180px">Dạng nhiên liệu</th>
                            <td>
                                <select className="custom-select fuelTypeId" id="fuelFormId" name="fuelFormId"
                                    onChange={(e) => changefuelFormId(e.target.value)} defaultValue={formik.values.fuelFormId}>
