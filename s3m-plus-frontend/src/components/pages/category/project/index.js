...
const ProjectComponent = () => {
    ...
    const [projectPath] = useState(useAppStore(state => state.categoryPath).find(f => f.path.includes("/category/project")));

    return (
        <Switch>
            {
                role === "ROLE_ADMIN" ? (
                    ProjectRouters.map((route, i) => {
                        let permissionActions = null;
                        if(route?.path === "/category/project"){
                            permissionActions = {
                        ...
                        return ((route?.path === "/category/project") ) ?  (
                    ...
                    ProjectRouters.map((route) => {
                        ...
                        return projectPath.actions.map((act, i) => {
                            if(route.action === act){
                                return ((path?.path === "/category/project") ) ? (
