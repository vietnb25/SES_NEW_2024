...
import { path } from "d3";
import ObjectTypeMstRouters from "./objectTypeMst.routes";

const ObjectTypeMstComponent = () => {
    const [role] = useState(AuthService.getRoleName());
    const [paths] = useState(useAppStore(state => state.categoryPath));
    const [objectTypePath] = useState(useAppStore(state => state.categoryPath).find(f => f.path.includes("/category/object-type-mst")));
    ...
    return (
        <Switch>
            {
                role === "ROLE_ADMIN" ? (
                    ObjectTypeMstRouters.map((route, i) => {
                        let permissionActions = null;
                        if (route?.path === "/category/object-type-mst") {
                            permissionActions = {
                        ...
                        return ((route?.path === "/category/object-type-mst")) ? (
                    ...
                    ObjectTypeMstRouters.map((route) => {
                        let permissionActions = null;
                        let path = paths.find(f => f.path === route.path);
                        let actions = path?.actions;
                        if (path?.path === "/category/object-type-mst") {
                            permissionActions = {
                        ...
                        return objectTypePath.actions.map((act, i) => {
                            if (route.action === act) {
                                return ((path?.path === "/category/object-type-mst")) ? (
...
export default ObjectTypeMstComponent;