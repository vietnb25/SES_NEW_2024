...
import "../select-device-sankey-3/select-device-sankey-3.css"
...
function SelectDeviceSankey3(props) {
    ...
    const [selectedDevicesTotal, setSelectedDevicesTotal] = useState([]);
    const [selectedDevicesClass3, setSelectedDevicesClass3] = useState([]);
    ...
    const sendData = (selectDeviceId, items, selectedDevicesClass3) => {
        props.parentCallback(selectDeviceId, items, selectedDevicesClass3)

    }

    const sendDataDeviceTotalAndDeviceClass3 = () => {
        let arr = [];
        ...
        setSelectedDevicesId((prev) => [...prev, arr])
        const newItem = {
            objectTotal: { selectedDevicesTotal },
            list: [selectedDevices],
        };

        setItems((prevItems) => [...prevItems, newItem]);
        setSelectedDevices([]);
        setSelectedDevicesTotal([]);
        getListDevice();
    }
    ...
    const getListDevice = async (type, area) => {
        ...
        const filteredList = res.data.filter(
            (fullItem) => !props.deviceClass2.some((removeItem) => removeItem.deviceId === fullItem.deviceId)
        );

        const filteredList2 = filteredList.filter(
            (fullItem) => !props.deviceClass1.some((removeItem) => removeItem.deviceId === fullItem.deviceId)
        );

        const filteredList3 = filteredList2.filter(
            (fullItem) => !props.deviceClass3.some((removeItem) => removeItem.deviceId === fullItem.deviceId)
        );
        if (selectedDevicesClass3 != "" || selectedDevicesClass3 != undefined) {
            const filteredList4 = filteredList3.filter(
                (fullItem) => !selectedDevicesClass3.some((removeItem) => removeItem.deviceId === fullItem.deviceId)
            );
            setDevices(filteredList4);
            setOldDevices(filteredList4);
        } else {
            setDevices(filteredList3);
            setOldDevices(filteredList3);
        }
    ...
    const onSelectDevice = (item) => {
        ...
        if (ck === false) {
            ...
            setSelectedDevicesClass3(Array.from(new Set([...selectedDevicesClass3, item])));
    ...
    const onSelectDeviceTotal = (item) => {
        let ck = false;
        for (let i = 0; i < selectedDevicesTotal.length; i++) {
            if (selectedDevicesTotal[i].deviceId == item.deviceId) {
                ck = true;
            }
        }
        if (ck === false) {
            setSelectedDevicesTotal(Array.from(new Set([...selectedDevicesTotal, item])));
        } else {
            // onUnSelectDevice(item);
        }
        // setChecked(item)
    ...
    return (
        <>
            {/* Modal */}
            <button type="button" className={'btn btn-select-device-sankey3'} data-toggle="modal" data-target="#exampleModalCenter1" onClick={() => {
                getListDevice()
            }
            } style={props.style} >
                {props.titleName}
                        ...
                        <div className='box-select'>
                            ...
                            <div className='modal-select-body row'>
                                ...
                                <div className='body-right col-4'>
                                    <p className='body-title-1'>{t('content.home_page.device_select.source_device')}</p>
                                    <div className='body-content'>
                                        {props.deviceClass3?.map((item, index) => {
                                            return (
                                                <div className="div-select" key={`selected` + index}
                                                    onClick={() => onSelectDeviceTotal(item)}
                                                >
                                                    <input type="checkbox" value={item.deviceId} className="" id={`ip1che1ck` + index} readOnly />
                                ...
                                <div className='body-right col-4'>
                                    <p className='body-title-1'>{t('content.home_page.device_select.device_lv3')}</p>
                                    <div className='body-content'>
                                        {selectedDevices?.map((item, index) => {
                                            return (
                                                <div className="div-select" key={`selected` + index}
                                                    onClick={() => onUnSelectDevice(item)}
                                                >
                                ...
                                <div className='body-left col-4'>
                                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                        <p className='body-title-1'>{t('content.list_device')}</p>
                                    </div>
                                    <div className='body-content'>
                                        {renderListDevice()}
                                    </div>
                            ...
                            <div className='modal-select-footer-sankey3'>
                                <button type="button" className="btn btn-confirm" style={{ marginRight: '1.5%' }} onClick={() => sendDataDeviceTotalAndDeviceClass3(selectedDevices)}>{t('content.home_page.device_select.save_device')}</button>
                                <button type="button" className="btn btn-confirm" style={{ marginRight: '1.5%' }} data-dismiss="modal" onClick={() => sendData(selectedDevicesId, items, selectedDevicesClass3)}>{t('content.accept')}</button>
...
export default SelectDeviceSankey3;