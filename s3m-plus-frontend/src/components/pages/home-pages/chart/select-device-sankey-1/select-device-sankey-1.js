...
import "../select-device-sankey-1/select-device-sankey-1.css"
...
function SelectDeviceSankey1(props) {
    ...
    const [selectedDevicesClass1, setSelectedDevicesClass1] = useState([]);
    ...
    const sendData = (selectDeviceId, items, selectedDevicesClass1) => {
        props.parentCallback(selectDeviceId, items, selectedDevicesClass1)

    }
    const sendDataDeviceTotalAndDeviceClass1 = () => {
        let arr = [];
    ...
    const getListDevice = async (type, area) => {
        ...
        if (selectedDevicesClass1 != "" || selectedDevicesClass1 != undefined) {
            const filteredList2 = filteredList.filter(
                (fullItem) => !selectedDevicesClass1.some((removeItem) => removeItem.deviceId === fullItem.deviceId)
            );
            setDevices(filteredList2);
            setOldDevices(filteredList2);
        } else {
            setDevices(filteredList);
            setOldDevices(filteredList);
        }
    }
    ...
    const onSelectDevice = (item) => {
        ...
        if (ck === false) {
            ...
            setSelectedDevicesClass1(Array.from(new Set([...selectedDevicesClass1, item])));
        } else {
    ...
    return (
        <>
            {/* Modal */}
            <button type="button" className={'btn btn-select-device-sankey1'} data-toggle="modal" data-target="#exampleModalCenter1" onClick={() => {
                getListDevice()
            }
            } style={props.style} >
                {props.titleName}
                ...
                <div id={"select-device-dialog"} className="modal-dialog modal-dialog-centered" role="document">
                    <div id={"select-device-content"} className="modal-content modal-select-content" >
                        <div className='box-select'>
                            ...
                            <div className='modal-select-body row'>
                                ...
                                <div className='body-right col-4'>
                                    ...
                                    <div className='body-content'>
                                        {props.deviceClass1?.map((item, index) => {
                                            return (
                                ...
                                <div className='body-right col-4'>
                                    <p className='body-title-1'>{t('content.home_page.device_select.device_lv1')}</p>
                                    <div className='body-content'>
                                ...
                                <div className='body-left col-4'>
                                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                                        ...
                                        {/* <div className="input-group" style={{ width: '45%', marginTop: '0' }}>
                                            <button type="button" className="btn btn-confirm" style={{ marginRight: '1.5%', height: 30 }} onClick={() => sendDataDeviceTotalAndDeviceClass1(selectedDevices)}>OK</button>
                                        </div> */}
                                    </div>
                                    <div className='body-content'>
                                        {renderListDevice()}
                            ...
                            <div className='modal-select-footer-sankey1'>
                                <button type="button" className="btn btn-confirm" style={{ marginRight: '1.5%' }} onClick={() => sendDataDeviceTotalAndDeviceClass1(selectedDevices)}>{t('content.home_page.device_select.save_device')}</button>
                                <button type="button" className="btn btn-confirm" style={{ marginRight: '1.5%' }} data-dismiss="modal" onClick={() => sendData(selectedDevicesId, items, selectedDevicesClass1)}>{t('content.accept')}</button>
...
export default SelectDeviceSankey1;