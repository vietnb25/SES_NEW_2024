    ...
    const drawChart = (dataChart, typeValue) => {
        ...
        dataChart.forEach(item => {
            if (item.pa && item.pa > 0) {
                item.pa = converter.convertElectricPower(typeValue, item.pa);
            }
            if (item.pb && item.pb > 0) {
                item.pb = converter.convertElectricPower(typeValue, item.pb);
            }
            if (item.pc && item.pc > 0) {
                item.pc = converter.convertElectricPower(typeValue, item.pc);
            }
        });
        ...
        am5.ready(function () {
            ...
            createSeries("Điện áp pha A", "uan", "circle");
            createSeries("Điện áp pha B", "ubn", "circle");
            createSeries("Điện áp pha C", "ucn", "circle");
