...
const ElectricalPower = () => {
    ...
    const getOperationInformation = async () => {
        ...
        let res = await OperationInformationService.getDataChart(param.customerId, param.deviceId, param.fromDate, null, param.pqsViewType, param.chartType);
    ...
    const drawChart = (dataChart, typeValue) => {
        ...
        dataChart.forEach(item => {
            if (item.low && item.low > 0) {
                item.low = converter.convertElectricPower(typeValue, item.low * 1000);
            } else {
                item.low = null;
            }
            if (item.normal && item.normal > 0) {
                item.normal = converter.convertElectricPower(typeValue, item.normal * 1000);
            } else  {
                item.normal = null;
            }
            if (item.high && item.high > 0) {
                item.high = converter.convertElectricPower(typeValue, item.high * 1000);
            } else {
                item.high = null;
            }
        });
        ...
        am5.ready(function () {
            ...
            yAxis.children.moveValue(am5.Label.new(root, { text: `Điện năng [[kWh]]`, rotation: -90, y: am5.p50, centerX: am5.p50, fontWeight: "bold" }), 0);
    ...
    }, [param.deviceId, param.fromDate, param.pqsViewType, param.chartType, param.customerId])
    return (
        <>
