...
const SettingWarning = ({projectInfo}) => {
    ...
    const [settingPath] = useState(useAppStore(state => state.categoryPath).find(f => f.path.includes("/home/load/:customerId/:projectId/setting")));
        ...
        <Switch>
            {
                role === "ROLE_ADMIN" ? (
                    SettingRouters.map((route, i) => {
                        ...
                        if(route?.path === "/home/load/:customerId/:projectId/setting"){
                            permissionActions = {
                        ...
                        return ((route?.path === "/home/load/:customerId/:projectId/setting") ) ?  (
                    ...
                    SettingRouters.map((route) => {
                        ...
                        return settingPath.actions.map((act, i) => {
                            if(route.action === act){
                                return ((path?.path === "/load/:customerId/:projectId/setting") ) ? (
