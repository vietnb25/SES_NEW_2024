...
const Manager = () => {
    ...
    const { customerId, superManagerId, managerId } = useParams();
    ...
    const getPowerByManagerId = async () => {

        let res = await HomeService.getAreaTree(param.customerId, param.superManagerId, param.managerId);
        if (res.status === 200) {
            setArea(res.data);
    ...
    const getDataChart = async () => {
        ...
        let res = await HomeService.chartManager(data);
    ...
    const download = async () => {
        ...
        $('#download').hide();
        setDownload(true)
        let res = await HomeService.downloadChartManager(data);
        if (res.status === 200) {
            setDownload(false)
            $('#download').show();
        }
    ...
    useEffect(() => {
        ...
        getPowerByManagerId();
        getDataChart();
    }, [param.customerId, param.superMangerId, param.managerId])
    return (
        <>
            <div className="tab-content">
                ...
                <table className="table tbl-overview tbl-power">
                    <thead style={area?.length >= 5 ? { display: "table", width: "calc(100% - 10px)", tableLayout: "fixed" } : { display: "table", width: "100%", tableLayout: "fixed" }}>
                        <tr>
