...
const AllCustomer = () => {
    ...
    const getPowerByCustomerId = async () => {
        let res = await HomeService.getAllCustomer();
        if (res.status === 200) {
            ...
            for (let index = 0; index < data.length; index++) {
                ...
                sumPvLoad += element.cspTotal;
                sumTotalPv += element.pvTotal;
            }
            setTotal({
                sumProjectNumber,
                sumPvLoad,
    ...
    const getDataChart = async () => {
        ...
        let res = await HomeService.chartAll(data);
    ...
    const handleSetViewTypeChart = (data) => {
        ...
        let viewChart = converter.setViewType(values.length > 0 ? min : 0);

        setViewTypeChart(viewChart);
    ...
    const download = async () => {
        ...
        $('#download').hide();
        setDownload(true)
        let res = await HomeService.downloadChartAll(data);
        if (res.status === 200) {
            setDownload(false)
            $('#download').show();
        }
        ...
        <>
            <div className="tab-content">
                ...
                <table className="table tbl-overview tbl-power">
                    <thead style={customer?.length >= 8 ? { display: "table", width: "calc(100% - 10px)", tableLayout: "fixed" } : { display: "table", width: "100%", tableLayout: "fixed" }}>
                        <tr>
                                ...
                                customer.map((item, index) => (
                                    <tr key={index} style={{ display: "table", width: "100%", tableLayout: "fixed" }}>
                                        ...
                                        <td width="71px" className="text-left" style={{ wordWrap: "break-word" }}>
                                            {/* <Link to={{
                                                pathname: "/",
                                                search: `?customerId=${item.customerId}&projectId=${item.projectId}`
                                            }} onClick={() => handleRedirect()}>{item.projectName}</Link> */}
