...
const OperationWarning = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async data => {
            ...
            let res = await OperationInformationService.updateOperatingWarningCache(updateWarningData);
            if (res.status === 200) {
    ...
    const getOperationWarning = async () => {
        ...
        let res = await OperationInformationService.getWarningOperation(param.deviceId, warningType, fromDate, toDate);
    ...
    const downloadWarning = async () => {
        ...
        let toDate = document.getElementById("to-date").value;
        let warningType = document.getElementById("warning-type").value;
        await OperationInformationService.downloadWarning(param.deviceId, warningType, fromDate, toDate);
    }
    ...
    const handleClickWarning = async (warningType, fromDate, toDate) => {
        let res = await OperationInformationService.showDataWarning(warningType, fromDate, toDate, param.deviceId, page);
        if (res.status === 200) {
            setDataLoadFrameWarning({ ...dataLoadFrameWarning, data: res.data })
            setIsModalOpen(true);
        }
    }

    const handleClickUpdate = async (warningId) => {
        let res = await OperationInformationService.getDetailOperatingWarningCache(warningId);
        if (res.status === 200) {
    ...
    const handleDownloadData = async (warningType, fromDate, toDate) => {
        let res = await OperationInformationService.download(warningType, fromDate, toDate, param.deviceId);
        if (res.status !== 200)
            ...
            <div id="main-search" className="ml-1 mb-3" style={{ height: '32px' }}>
                ...
                <div className="input-group float-left mr-1" style={{ width: '270px' }}>
                    ...
                    <select id="warning-type" name="dataType" defaultValue={CONS.WARNING_TYPE.QUA_TAI} className="custom-select block custom-select-sm" onChange={() => getOperationWarning()}>
                        ...
                        <option value={CONS.WARNING_TYPE.NGUONG_TONG_HAI}>Sóng hải</option>
                    </select>
                </div>
                <div className="search-buttons float-left">
                    ...
                    <button type="button" className="btn btn-outline-secondary btn-sm" onClick={() => downloadWarning()}>
                    ...
                    <tbody>
                        {
                            warnings.map((warning, index) => (
                                <tr>
                                    ...
                                    <td>
                                        <Link to={`/load/${warning.projectId}/systemMap/${warning.systemMapId}?deviceId=${param.deviceId}`} className="button-icon float-right" title="View Grid">
                                             {warning.systemMapName}  
                                            </Link>
                                        </td>
                                    <td onClick={() => {
                                        setWarningType(warning.warningType);
                                    ...
                                    <td>
                                    <a className="button-icon text-left" onClick={() =>
                                        ...
                                        <a className="button-icon float-right" onClick={() => {
                        ...
                        warningType !== 103 ?
                                        ...
                                        return (
                                            <React.Fragment key={index}>
                                                <tr className="text-center" >
                                                    ...
                                                    <td>{warning.thdIa}</td>
                                                    <td rowSpan="3">{warning.f}</td>
                                                    <td>-</td>
                                                    <td>-</td>
                                                    <td rowSpan="3">{warning.ep}</td>
                                                </tr>
                                                <tr className="text-center">
                                                    <td >B</td>
                                                    <td>{warning.ubn}</td>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <th width="150px">Môi trường</th>
                                    <th width="150px">Trong tủ</th>
                                    <th width="150px">Dầu</th>
                                    <th width="150px">Pha</th>
                                    <th width="150px">Đầu cực</th>
                                    <th width="150px">Vị trí 1</th>
                                    <th width="150px">Vị trí 2</th>
                                </tr>
                                {
                                    dataLoadFrameWarning.data.map((warning, index) => {
                                        return <React.Fragment key={index}>
                                            <tr className="text-center" >
                                                <td rowSpan="3">{index + 1}</td>
                                                <td rowSpan="3">{warning.sentDate}</td>
                                                <td rowSpan="3">-</td>
                                                <td rowSpan="3">-</td>
                                                <td rowSpan="3">-</td>
                                                <td>A</td>
                                    ...
                                    <div className="form-control p-0">
                                        <div className="dropdown text-left">
                                            <select id="status" name="status" className="custom-select block" style={{ borderRadius: 0 }}
                                                defaultValue={updateWarning.status}
