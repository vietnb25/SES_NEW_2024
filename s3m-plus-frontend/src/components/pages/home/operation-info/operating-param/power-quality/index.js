...
const PowerQuality = () => {
    ...
    const getPowerQualities = async () => {
        ...
        let res = await OperationInformationService.getPowerQuality(param.deviceId, param.fromDate, param.toDate);
        if (res.status === 200 && parseInt(res.data.length) > 0) {
            $('#loading').hide();
            $('.table-data').show();
            let powerQualities = res.data;
    ...
    useEffect(() => {
        getPowerQualities();
    }, [param.deviceId, param.fromDate, param.toDate])
    return (
        <>
            ...
            </div>
            {
                ...
                <div className="tab-content">
                    <table className="table tbl-overview tbl-tsd mt-3" style={{ marginLeft: "0" }}>
                        <thead>
                            <tr>
                                <th colspan={34} className="tbl-title">Thông số chất lượng điện năng [%]</th>
                            </tr>
                        </thead>
                    </table>
