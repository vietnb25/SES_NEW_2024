...
const Area = () => {
    ...
    const { customerId, superManagerId, managerId, areaId } = useParams();
    ...
    const getPowerByAreaId = async () => {
        let res = await HomeService.getProjectByTree(param.customerId, param.superManagerId, param.managerId, param.areaId);
        if (res.status === 200) {
            setProject(res.data);
            ...
            for (let index = 0; index < data.length; index++) {
                ...
                sumProjectNumber += element.deviceNumber;
                sumPvLoad += element.cspTotal;
                sumTotalPv += element.pvValue;
            }
            setTotal({
                sumProjectNumber,
                sumPvLoad,
    ...
    const getDataChart = async () => {
        ...
        let res = await HomeService.chartArea(data);
    ...
    const download = async () => {
        ...
        $('#download').hide();
        setDownload(true)
        let res = await HomeService.downloadChartArea(data);
        if (res.status === 200) {
            setDownload(false)
            $('#download').show();
        }
    ...
    useEffect(() => {
        ...
        getPowerByAreaId();
        getDataChart();
    }, [param.customerId, param.superMangerId, param.managerId, param.areaId])
    return (
        <>
