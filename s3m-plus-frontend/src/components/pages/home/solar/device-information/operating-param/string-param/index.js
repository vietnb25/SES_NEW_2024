...
const StringParam = () => {
    ...
    const getOperationInformation = async () => {
        ...
        $('#no-data').hide();
        $('#loading').show();
        let res = await OperationInformationService.getOperationStringPV(param.customerId, param.deviceId, param.fromDate, param.toDate, page);
                                    ...
                                    operationInfo?.map((item, index) => (
                                        <React.Fragment key={index + 1}>
                                            <tr>
                                                ...
                                                <td className="text-center" height="70px">{item?.vdcStr != null ? item?.vdcStr : "-"}</td>
                                                <td className="text-center" height="70px">{item?.idcStr != null ? item?.idcStr : "-"}</td>
                                                <td className="text-center" height="70px">{item?.pdcStr != null ? item?.pdcStr : "-"}</td>
                                                <td className="text-center" height="70px">{item?.epStr != null ? item?.epStr : "-"}</td>
                                                <td className="text-center" height="70px">{item?.tstr != null ? item?.tstr : "-"}</td>
