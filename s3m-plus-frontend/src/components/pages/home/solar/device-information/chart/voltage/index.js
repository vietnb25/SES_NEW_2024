...
const Voltage = () => {
    ...
    const getOperationInformation = async () => {
        ...
        switch (deviceType) {
            ...
            case CONS.DEVICE_TYPE_PV.PANEL:
                ...
                res = await OperationInformationService.getDataChartPanelPV(param.customerId, param.deviceId, param.fromDate, param.toDate);
                if (res.status === 200 && parseInt(res.data.length) > 0) {
                    $('#loading').hide();
                    $('#no-data').hide();
                    drawChart(res.data);
    ...
    const drawChart = (dataChart) => {
        ...
        am5.ready(function () {
            ...
            yAxis.children.moveValue(am5.Label.new(root, { text: "Điện áp (V)", rotation: -90, y: am5.p50, centerX: am5.p50, fontWeight: "bold" }), 0);
            ...
            if (parseInt(param.deviceType) === CONS.DEVICE_TYPE_PV.INVERTER) {
                createSeries("Điện áp pha A", "va", "circle");
                createSeries("Điện áp pha B", "vb", "circle");
                createSeries("Điện áp pha C", "vc", "circle");
                createSeries("Điện áp DC", "udc", "circle");
            }
            if (parseInt(param.deviceType) === CONS.DEVICE_TYPE_PV.COMBINER) {
                createSeries("Điện áp DC", "vdcCombiner", "circle");
            }
            if (parseInt(param.deviceType) === CONS.DEVICE_TYPE_PV.STRING) {
                createSeries("Điện áp DC", "vdcStr", "circle");
            }
            if (parseInt(param.deviceType) === CONS.DEVICE_TYPE_PV.PANEL) {
                createSeries("Điện áp DC", "u", "circle");
            }
