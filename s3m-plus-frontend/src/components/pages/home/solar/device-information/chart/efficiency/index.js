...
const Efficiency = () => {
    ...
    const getOperationInformation = async () => {
        ...
        if (deviceType === CONS.DEVICE_TYPE_PV.INVERTER) {
            let res = await OperationInformationService.getDataChartInverterPV(param.customerId, param.deviceId, param.fromDate, param.toDate);
    ...
    const drawChart = (dataChart) => {
        ...
        am5.ready(function () {
            ...
            yAxis.children.moveValue(am5.Label.new(root, { text: "Hiệu suất [[W/m2]]", rotation: -90, y: am5.p50, centerX: am5.p50, fontWeight: "bold" }), 0);
            ...
            if (parseInt(param.chartType) === CONS.CHART_TYPE_PV.BUC_XA) {
                createSeries("Bức xạ", "rad", "circle", "[[W/m2]]");
            }

            if (parseInt(param.chartType) === CONS.CHART_TYPE_PV.NHIET_DO) {
                createSeries("Nhiệt độ", "temp", "circle", "[[°C]]");
            }

            if (parseInt(param.chartType) === CONS.CHART_TYPE_PV.TOC_DO_GIO) {
                createSeries("Tốc độ gió", "windSp", "circle", "[[m/s]]");
            }

            if (parseInt(param.chartType) === CONS.CHART_TYPE_PV.AP_SUAT) {
                createSeries("Áp suất", "atmos", "circle", "[[atm]]");
            }
