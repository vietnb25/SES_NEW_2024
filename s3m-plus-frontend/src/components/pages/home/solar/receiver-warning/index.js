...
const ReceiverWarning = ({ projectInfo, projectId }) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (data) => {
            let res = await receiverService.addNewReceiverPV(data, projectId, 2);
            if (res.status === 200) {
                $('#new-receiver').hide();
                getResetReceiver();
                getListReceiver();
    ...
    const formikUpdate = useFormik({
        ...
        onSubmit: async (data) => {
            let res = await receiverService.updateReceiverPV(data);
            if (res.status === 200) {
    ...
    const getListDevice = async (receiverId) => {
        let res = await DeviceService.listDevice(projectId, 2);
        if (res.status === 200) {
            setDevices(res.data)
        }
    ...
    const getListReceiver = async () => {
        let res = await receiverService.listReceiverPV(projectId, CONS.SYSTEM_TYPE.PV);
        if (res.status === 200) {
    ...
    const onLoadDevice = async (_deviceId, receiverId) => {
        ...
        let res = await receiverService.getWarningInforPV(receiverId, _deviceId);
        let _warningRes = [];
        warning?.forEach(e => {
    ...
    const save = async (receiverId) => {
        let res = await receiverService.saveWarningInforPV(warningSelected.map(item => item.warningType).toString(), CONS.SYSTEM_TYPE.LOAD, param.customerId, param.projectId, deviceId, receiverId);
        if (res.status === 200) {
    ...
    const accept = async (receiverId) => {
        let res = await receiverService.deleteReceiverPV(receiverId);
        if (res.status === 200) {
