...
const SuperManager = () => {
    ...
    const { customerId, superManagerId } = useParams();
    ...
    const getPowerBySuperManagerId = async () => {

        let res = await HomeService.getManagerTree(param.customerId, param.superManagerId);
        if (res.status === 200) {
            setManager(res.data);
            ...
            for (let index = 0; index < data.length; index++) {
                ...
                sumProjectNumber += element.areaNumber;
            ...
            setProjectInfor(data[0].infoProject);
    ...
    const getDataChart = async () => {
        ...
        let res = await HomeService.chartSuperManager(data);
    ...
    const download = async () => {
        ...
        $('#download').hide();
        setDownload(true)
        let res = await HomeService.downloadChartSuperManager(data);
        if (res.status === 200) {
            setDownload(false)
            $('#download').show();
        }
    ...
    useEffect(() => {
        document.title = "Home";
        getDateNewDevice();
        getPowerBySuperManagerId();
        getDataChart();
    }, [param.customerId, param.superManagerId])
    return (
        <>
            <div className="tab-content">
                ...
                <table className="table tbl-overview tbl-power">
                    <thead style={manager?.length >= 5 ? { display: "table", width: "calc(100% - 10px)", tableLayout: "fixed" } : { display: "table", width: "100%", tableLayout: "fixed" }}>
                        <tr>
