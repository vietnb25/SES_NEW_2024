...
const ReceiverWarning = ({ projectInfo, projectId }) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (data) => {
            let res = await receiverService.addNewReceiverGrid(data, projectId, 5);
            if (res.status === 200) {
                $('#new-receiver').hide();
                getResetReceiver();
                getListReceiver();
    ...
    const formikUpdate = useFormik({
        ...
        onSubmit: async (data) => {
            let res = await receiverService.updateReceiverGrid(data);
    ...
    const getListDevice = async (receiverId) => {
        let res = await DeviceService.listDevice(projectId, 5);
        if (res.status === 200) {
            setDevices(res.data)
        }
    ...
    const getListReceiver = async () => {
        let res = await receiverService.listReceiverGrid(projectId, CONS.SYSTEM_TYPE.GRID);
        if (res.status === 200) {
    ...
    const onLoadDevice = async (_deviceId, receiverId) => {
        ...
        let res = await receiverService.getWarningInforGrid(receiverId, _deviceId);
        let _warningRes = [];
        warning?.forEach(e => {
    ...
    const save = async (receiverId) => {
        let res = await receiverService.saveWarningInforGrid(warningSelected.map(item => item.warningType), CONS.SYSTEM_TYPE.LOAD, param.customerId, param.projectId, deviceId, receiverId);
        if (res.status === 200) {
    ...
    const accept = async (receiverId) => {
        let res = await receiverService.deleteReceiverGrid(receiverId);
        if (res.status === 200) {
                                    ...
                                    <table className="table table-input">
                                        <tbody>
                                            ...
                                            <tr>
                                                <th width="150px">Số điện thoại<span className="required">※</span></th>
                                                <td>
                                                    <input type="text" pattern="[0-9]{0,}" className="form-control" id="phone" name="phone" maxLength={255} onChange={formik.handleChange} />
                                                </td>
                                            </tr>
                                            <tr>
                                                <th width="150px">Email<span className="required">※</span></th>
