...
const ReportGrid = ({ projectInfo }) => {
  ...
  const download = async (url) => {
    let res = await reportService.downloadReportGrid(url);
    if (res.status != 200) {
      $.alert({
        title: 'Thông báo!',
  ...
  const reportDetail = async () => {
    let res = await reportService.getReportGrid(userName, projectId);
    if (res.status == 200) {
  ...
  const getReportName = (reportId) => {
    let reportName = "";
    if (reportId == 1) {
      return reportName = "Báo cáo trong tổng hợp trong "
    } else if (reportId == 2) {
  ...
  const addReport = async () => {
    ...
    let reportType = document.getElementById("reportTyte").value;
    if (dateType == 1) {
    ...
    let response = await reportService.addReportGrid(data, userName, customerId);
    if (response.status == 200) {
      reportDetail();
      let res = await reportService.generateReportsGrid(data, userName, customerId, profiledata);
      if (res.status == 200) {
        reportDetail();
      }
    } else {
