...
import vn.ses.s3m.plus.service.ForecastService;
...
public class ForecastController {
	...
	@GetMapping("/forecasts/{projectId}/{systemTypeId}/{page}")
	public ResponseEntity<?> getForecasts(@PathVariable Long projectId, @PathVariable Long systemTypeId,
	...
	@GetMapping("/forecast/{projectId}/{systemTypeId}")
	public ResponseEntity<?> getForecast(@PathVariable Long projectId, @PathVariable Long systemTypeId) {
		Map<String, Object> condition = new HashMap<>();
		condition.put("projectId", projectId);
		condition.put("systemTypeId", systemTypeId);
		...
		if (forecast != null) {
			...
			return new ResponseEntity<Forecast>(forecastNull, HttpStatus.OK);
		}
	}
	@PostMapping("/forecast/save")
	public ResponseEntity<?> saveForecast(@RequestBody Forecast forecast){	
